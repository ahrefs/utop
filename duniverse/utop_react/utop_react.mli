...
(** Declarative events and signals.
    ...
    Utop_react is a module for functional reactive programming (frp).  It
    provides support to program with time varying values : declarative
    {{!E}events} and {{!S}signals}. Utop_react
...
(** {1:sem Semantics}
    ...
    Utop_react doesn't define primitive events and signals, they must be
...
let () =
  ...
  Step.execute step (* w x z y occur simultaneously *)
    ...
    value, Utop_react performs an update step.
...
open Stdlib (* back to Stdlib floats *)
   ...
   construct
{[let open Utop_react.S.Float in
