...
val fill : context -> ?style : LTerm_style.t -> Utop_zed_char.t -> unit
...
val draw_char_matrix : matrix -> int -> int -> ?style : LTerm_style.t -> Utop_zed_char.t -> unit
...
val draw_char : context -> int -> int -> ?style : LTerm_style.t -> Utop_zed_char.t -> unit
  (** [draw_char ctx row column ?style ch] sets the character at given
      coordinates to [ch]. It does nothing if the given coordinates
      are outside the bounds of the context. *)

val draw_string : context -> int -> int -> ?style : LTerm_style.t -> Utop_zed_string.t -> unit
...
val draw_string_aligned : context -> int -> horz_alignment -> ?style : LTerm_style.t -> Utop_zed_string.t -> unit
...
val draw_frame_labelled : context -> rect ->
  ...
  Utop_zed_string.t -> connection -> unit
