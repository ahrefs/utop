...
let space = Utop_zed_char.unsafe_of_char ' '
let newline = Utop_zed_char.unsafe_of_char '\n'
  ...
  Array.init
    ...
    (fun _ ->
       Array.init
         size.cols
         (fun _ -> ref @@ Elem {
            char = Utop_zed_char.unsafe_of_char ' ';
            bold = false;
            underline = false;
            blink = false;
            reverse = false;
...
let unsafe_del matrix row col len=
  ...
  let rec find col=
    ...
    match !point with
    | Elem elem->
      let width= Utop_zed_char.width elem.char in
...
let draw_string ctx row col ?style str=
  let len= Utop_zed_string.bytes str in
...
let draw_string_aligned ctx row alignment ?style str=
  ...
  in
  let line_width start= actual_width (Utop_zed_string.width_ofs ~start str) in
  let rec loop row col ofs=
    if ofs < Utop_zed_string.bytes str then begin
      let ch, ofs= Utop_zed_string.extract_next str ofs in
      if ch = newline then
        ofs
      else begin
...
let draw_styled_aligned ctx row alignment ?style str=
  let str, styles=
    ...
    Utop_zed_string.implode (Array.to_list (Array.init len (fun i-> fst (Array.get str i))))
  ...
  let rec loop row col idx ofs=
    if ofs < Utop_zed_string.bytes str then begin
      let (ch, ofs), ch_style=
        Utop_zed_string.extract_next str ofs, Array.unsafe_get styles idx
      and idx= idx + 1 in
      if ch = newline then
        (idx, ofs)
      else begin
        ...
        draw_char_raw ctx row col ?style ch;
        set_style point ch_style;
        loop row (col + Utop_zed_char.width ch) idx ofs;
      end
    end else
      (idx, ofs)
  in
...
let piece_of_point point=
  match !point with
  | Elem elem-> piece_of_char (Utop_zed_char.core elem.char)
  | WidthHolder _-> None
  ...
  if row >= ctx.ctx_row1 && col >= ctx.ctx_col1 && row < ctx.ctx_row2 && col < ctx.ctx_col2 then begin
      ...
      if row > 0 then begin
        ...
        | Some piece' ->
          if piece.top = piece'.bottom then
            ...
            let char= Utop_zed_char.unsafe_of_uChar
    ...
    let piece=
      if col < ctx.ctx_matrix_size.cols - 1 then begin
        ...
        piece
    in
    let char= Utop_zed_char.unsafe_of_uChar (char_of_piece piece) in
