...
type action =
  | Utop_zed of Utop_zed_edit.action
  | Start_macro
  | Stop_macro
  | Cancel_macro
  | Play_macro
...
let doc_of_action = function
  | Utop_zed action -> Utop_zed_edit.doc_of_action action
...
let action_of_name x =
  let rec loop a b =
    if a = b then
      Utop_zed (Utop_zed_edit.action_of_name x)
    else
      let c = (a + b) / 2 in
...
let name_of_action x =
  ...
  in
  match x with
    | Utop_zed x -> Utop_zed_edit.name_of_action x
    | Custom _ -> "custom"
...
let () =
  bind [{ control = false; meta = false; shift = false; code = Left }] [Utop_zed Prev_char];
  bind [{ control = false; meta = false; shift = false; code = Right }] [Utop_zed Next_char];
  bind [{ control = false; meta = false; shift = false; code = Up }] [Utop_zed Prev_line];
  bind [{ control = false; meta = false; shift = false; code = Down }] [Utop_zed Next_line];
  bind [{ control = false; meta = false; shift = false; code = Home }] [Utop_zed Goto_bol];
  bind [{ control = false; meta = false; shift = false; code = End }] [Utop_zed Goto_eol];
  bind [{ control = false; meta = false; shift = false; code = Insert }] [Utop_zed Switch_erase_mode];
  bind [{ control = false; meta = false; shift = false; code = Delete }] [Utop_zed Delete_next_char];
  bind [{ control = false; meta = false; shift = false; code = Enter }] [Utop_zed Newline];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char ' ') }] [Utop_zed Set_mark];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'a') }] [Utop_zed Goto_bol];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'e') }] [Utop_zed Goto_eol];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'd') }] [Utop_zed Delete_next_char];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'h') }] [Utop_zed Delete_prev_char];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'k') }] [Utop_zed Kill_next_line];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'u') }] [Utop_zed Kill_prev_line];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'n') }] [Utop_zed Next_line];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'p') }] [Utop_zed Prev_line];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'w') }] [Utop_zed Kill];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'y') }] [Utop_zed Yank];
  bind [{ control = false; meta = false; shift = false; code = Backspace }] [Utop_zed Delete_prev_char];
  bind [{ control = false; meta = true; shift = false; code = Char(Uchar.of_char 'w') }] [Utop_zed Copy];
  bind [{ control = false; meta = true; shift = false; code = Char(Uchar.of_char 'c') }] [Utop_zed Capitalize_word];
  bind [{ control = false; meta = true; shift = false; code = Char(Uchar.of_char 'l') }] [Utop_zed Lowercase_word];
  bind [{ control = false; meta = true; shift = false; code = Char(Uchar.of_char 'u') }] [Utop_zed Uppercase_word];
  bind [{ control = false; meta = true; shift = false; code = Char(Uchar.of_char 'b') }] [Utop_zed Prev_word];
  bind [{ control = false; meta = true; shift = false; code = Char(Uchar.of_char 'f') }] [Utop_zed Next_word];
  bind [{ control = false; meta = true; shift = false; code = Right }] [Utop_zed Next_word];
  bind [{ control = false; meta = true; shift = false; code = Left }] [Utop_zed Prev_word];
  bind [{ control = true; meta = false; shift = false; code = Right }] [Utop_zed Next_word];
  bind [{ control = true; meta = false; shift = false; code = Left }] [Utop_zed Prev_word];
  bind [{ control = false; meta = true; shift = false; code = Backspace }] [Utop_zed Kill_prev_word];
  bind [{ control = false; meta = true; shift = false; code = Delete }] [Utop_zed Kill_prev_word];
  bind [{ control = true; meta = false; shift = false; code = Delete }] [Utop_zed Kill_next_word];
  bind [{ control = false; meta = true; shift = false; code = Char(Uchar.of_char 'd') }] [Utop_zed Kill_next_word];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char '_') }] [Utop_zed Undo];
...
let clipboard = Utop_zed_edit.new_clipboard ()
let macro = Utop_zed_macro.create []

let dummy_engine = Utop_zed_edit.create ()
let dummy_cursor = Utop_zed_edit.new_cursor dummy_engine
let dummy_context = Utop_zed_edit.context dummy_engine dummy_cursor
let newline = Utop_zed_char.unsafe_of_uChar (Uchar.of_char '\n')
...
object(self)
  ...
  method text = Utop_zed_rope.to_string (Utop_zed_edit.text engine)
  ...
  method private update_window_position =
    let line_set = Utop_zed_edit.lines engine in
    let line_count = Utop_zed_lines.count line_set in
    let cursor_offset = Utop_zed_cursor.get_position cursor in
    let cursor_line = Utop_zed_lines.line_index line_set cursor_offset in
    let cursor_column = cursor_offset - Utop_zed_lines.line_start line_set cursor_line in
    let column_display= Utop_zed_lines.force_width line_set (Utop_zed_lines.line_start line_set cursor_line) cursor_column in
    ...
    (* Vertical check *)
    let start_line' = Utop_zed_lines.line_index line_set start in
    let start_line' =
      if cursor_line < start_line' || cursor_line >= start_line' + size.rows then begin
        ...
        start <- Utop_zed_lines.line_start line_set start_line';
        start_line'
      end else
        start_line'
    in
  ...
  initializer
    ...
    cursor <- Utop_zed_edit.new_cursor engine;
    context <- Utop_zed_edit.context engine cursor;
    Utop_zed_edit.set_data engine (self :> edit);
    event <- E.map (fun _ ->
      self#update_window_position;
      self#queue_draw) (Utop_zed_edit.update engine [cursor]);
    self#on_event
      (function
             ...
             match Bindings.resolve key res with
               | Bindings.Accepted actions ->
                   ...
                   let rec exec = function
                     | Custom f :: actions ->
                         Utop_zed_macro.add macro (Custom f);
                         f ();
                         exec actions
                     | Utop_zed action :: actions ->
                         Utop_zed_macro.add macro (Utop_zed action);
                     ...
                     | Play_macro :: actions ->
                         Utop_zed_macro.cancel macro;
                         exec (Utop_zed_macro.contents macro @ actions)
                     | Insert_macro_counter :: actions ->
                         Utop_zed_macro.add macro Insert_macro_counter;
  ...
  initializer vscroll#on_offset_change (fun n ->
    ...
    if cursor_line < start_line then begin
      ...
      Utop_zed_edit.move_line context d (* first row *)
      ...
      Utop_zed_edit.move_line context d
    end;
    self#queue_draw;
  )
  ...
  method! draw ctx _focused =
    ...
    fill ctx (Utop_zed_char.unsafe_of_char ' ');
    fill_style ctx style;

    (*** Text drawing ***)

    let rec draw_line row col zip =
      if Utop_zed_rope.Zip.at_eos zip then
        draw_eoi (row + 1)
      else
        let char, zip = Utop_zed_rope.Zip.next zip in
        if char = newline then begin
          ...
          if col > size.cols then begin
            ...
            draw_line row (col + (Utop_zed_char.width char)) zip
          end
        end
    ...
    and skip_bol row zip remaining =
      ...
      else if Utop_zed_rope.Zip.at_eos zip then
        draw_eoi (row + 1)
      else
        let char, zip = Utop_zed_rope.Zip.next zip in
        if char = newline then begin
          ...
          skip_bol row zip (remaining - (Utop_zed_char.width char))
    ...
    let text = Utop_zed_edit.text engine in

    begin_line 0 (Utop_zed_rope.Zip.make_f text start);
    ...
    if Utop_zed_edit.get_selection engine then begin
      let sel_offset = Utop_zed_cursor.get_position (Utop_zed_edit.mark engine) in
      let sel_line = Utop_zed_lines.line_index line_set sel_offset in
      let sel_column = sel_offset - Utop_zed_lines.line_start line_set sel_line in
  ...
  method! cursor_position =
    ...
    let cursor_offset = Utop_zed_cursor.get_position cursor in
    let cursor_line = Utop_zed_lines.line_index line_set cursor_offset in
    let line_start= Utop_zed_lines.line_start line_set cursor_line  in
    let start_line = Utop_zed_lines.line_index line_set start in
    let col= Utop_zed_lines.force_width line_set line_start (cursor_offset - line_start)
        - shift_width in
