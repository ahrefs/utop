...
module Make (LiteralIntf: LiteralIntf.Type) = struct
  ...
  type t = (Utop_zed_char.t * LTerm_style.t) array

  (* +-----------------------------------------------------------------+
    | Conversions                                                     |
    +-----------------------------------------------------------------+ *)

  let dummy = (Utop_zed_char.unsafe_of_char ' ', LTerm_style.none)

  let of_string str=
    Array.map (fun chr-> (chr, LTerm_style.none)) (Array.of_list (Utop_zed_string.unsafe_explode str))

  let aval_width= function
    | Ok {Utop_zed_string.len=_;width}-> width
    | Error {Utop_zed_string.start=_;len=_;width}-> width

  let of_utf8 str=
    let str= Utop_zed_string.unsafe_of_utf8 str in
    of_string str

  (*let rec invalid_length str ofs acc =
    let ofs, len, _ = Utop_zed_utf8.next_error str ofs in
  ...
  let of_string_maybe_invalid str=
    let len= Utop_zed_string.length str in
    ...
    let rec loop ofs idx=
      if idx = len then
        ...
        let ofs, idx=
          try
            let chr, ofs= Utop_zed_string.extract_next str ofs in
            Array.unsafe_set arr idx (chr, LTerm_style.none);
            (ofs, idx + 1)
          with
            Utop_zed_utf8.Invalid _->
            let code= Uchar.to_int (Utop_zed_char.core (Utop_zed_string.extract str ofs)) in
            Array.unsafe_set arr (idx + 0)
              (Utop_zed_char.unsafe_of_char '\\', LTerm_style.none);
            Array.unsafe_set arr (idx + 1)
              (Utop_zed_char.unsafe_of_char 'y', LTerm_style.none);
            Array.unsafe_set arr (idx + 2)
              (Utop_zed_char.unsafe_of_uChar (uchar_of_hex (code lsr 4))
              , LTerm_style.none);
            Array.unsafe_set arr (idx + 3)
              (Utop_zed_char.unsafe_of_uChar (uchar_of_hex (code land 15))
              , LTerm_style.none);
            ofs + 1, idx + 4
        in
        loop ofs idx
  ...
  let to_string txt =
    Utop_zed_string.init (Array.length txt) (fun i-> fst txt.(i))

  let of_rope rope =
    let arr = Array.make (Utop_zed_rope.length rope) dummy in
    let rec loop zip idx=
      if Utop_zed_rope.Zip.at_eos zip then
        arr
      else begin
        let chr, zip = Utop_zed_rope.Zip.next zip in
        Array.unsafe_set arr idx (chr, LTerm_style.none);
        loop zip (idx + 1)
      end
    in
    loop (Utop_zed_rope.Zip.make_f rope 0) 0

  let to_rope txt =
    let buf = Utop_zed_rope.Buffer.create () in
    Array.iter (fun (ch, _style) -> Utop_zed_rope.Buffer.add buf ch) txt;
    Utop_zed_rope.Buffer.contents buf

  let stylise str style =
    let str= Utop_zed_string.unsafe_of_utf8 str in
    Array.map (fun chr-> (chr, style)) (Array.of_list (Utop_zed_string.explode str))

  (* +-----------------------------------------------------------------+
    | Parenthesis matching                                            |
    +-----------------------------------------------------------------+ *)

  let lparen = Utop_zed_char.unsafe_of_char '('
  let rparen = Utop_zed_char.unsafe_of_char ')'
  let lbrace = Utop_zed_char.unsafe_of_char '{'
  let rbrace = Utop_zed_char.unsafe_of_char '}'
  let lbracket = Utop_zed_char.unsafe_of_char '['
  let rbracket = Utop_zed_char.unsafe_of_char ']'
  ...
  let markup_length markup =
    let rec loop len = function
      | [] -> len
      | S str :: rest -> loop (len + Utop_zed_string.length (LiteralIntf.to_string_exn str)) rest
      | R str :: rest -> loop (len + Utop_zed_rope.length str) rest
  ...
  let eval markup =
    ...
    let rec copy_string str ofs idx style =
      if ofs >= Utop_zed_string.bytes str then
        idx
      else begin
        let chr, ofs= Utop_zed_string.extract_next str ofs in
    ...
    let rec loop idx style = function
      ...
      | R str :: rest ->
          loop (copy_rope (Utop_zed_rope.Zip.make_f str 0) idx style) style rest
      | B_bold status :: rest ->
