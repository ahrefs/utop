...
let yspace = Utop_zed_char.unsafe_of_uChar uspace
...
let encode_char ch = Utop_zed_utf8.singleton ch
  ...
  if Array.length text = 0 then
    ...
    let rec loop idx prev_style =
      if idx = Array.length text then begin
        ...
        Buffer.add_string buf (Utop_zed_char.to_utf8 ch);
        loop (idx + 1) style
      end
    in
...
let fprints_windows term oc text =
  let rec loop idx prev_attr =
    if idx = Array.length text then begin
      ...
      let chars= Utop_zed_char.to_raw ch in
      let s= chars
...
let fprints term text =
  if term.outgoing_is_a_tty then
    ...
    fprint term (Utop_zed_string.to_utf8 (LTerm_text.to_string text))
...
let unknown_char = Utop_zed_char.unsafe_of_uChar unknown_uchar
let unknown_utf8 = Utop_zed_char.to_utf8 unknown_char
...
let render_point term buf old_point new_point =
  ...
  if Uchar.to_int (Utop_zed_char.core new_point.LTerm_draw.char) < 32 then
    Buffer.add_string buf unknown_utf8
  else
    Buffer.add_string buf (Utop_zed_char.to_utf8 new_point.LTerm_draw.char)
        ...
        let rec loop i =
          if i = len then
            ...
            | LTerm_draw.Elem point->
              let code = Uchar.to_int (Utop_zed_char.core point.LTerm_draw.char) in
...
let print_box_with_newlines_unix term matrix =
  ...
  for y = 0 to rows - 1 do
    ...
    let rec loop x =
      ...
      | Elem point->
        let code = Uchar.to_int (Utop_zed_char.core point.char) in
