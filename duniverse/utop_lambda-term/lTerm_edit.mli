...
val actions : (action * string) list
  (** List of actions with their names, except {!Utop_zed}. *)
...
val clipboard : Utop_zed_edit.clipboard
...
(** Class of edition widgets. If no clipboard is provided, then the
    global one is used. *)
class edit :
  ?clipboard : Utop_zed_edit.clipboard ->
  ?macro : action Utop_zed_macro.t ->
  ?size : LTerm_geom.size -> unit -> object
  inherit LTerm_widget.t

  method engine : edit Utop_zed_edit.t
    (** The edition engine used by this widget. *)

  method cursor : Utop_zed_cursor.t
    (** The cursor used by this widget. *)

  method context : edit Utop_zed_edit.context
  ...
  method text : Utop_zed_string.t
    (** Shorthand for [Utop_zed_rope.to_string (Utop_zed_edit.text
        edit#engine)]. *)

  method editable : int -> int -> bool
    (** The editable function of the engine. *)

  method match_word : Utop_zed_rope.t -> int -> int option
