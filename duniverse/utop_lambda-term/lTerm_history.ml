...
type node = {
  mutable data : Utop_zed_string.t;
  mutable size : int;
  mutable prev : node;
}
...
type t = {
  ...
  mutable cache : Utop_zed_string.t list option;
  (* When set, the cache is equal to the list of entries, from the
     most recent to the oldest. *)
}

let entry_size str =
  let zChar_newline= Utop_zed_char.unsafe_of_char '\n'
  and zChar_slash= Utop_zed_char.unsafe_of_char '\\' in
  let size = ref 0 in
  let eos= Utop_zed_string.bytes str in
  let rec calc ofs=
    if ofs < eos then
      let ch, ofs= Utop_zed_string.extract_next str ofs in
      if Utop_zed_char.compare ch zChar_newline = 0 || Utop_zed_char.compare ch zChar_slash = 0 then
        size := !size + 2
      else
        size := !size + 1;
      calc ofs
    ...
    match entries with
      ...
      | entry :: entries ->
          ...
          if size_ok size entry_size max_size && count + 1 < max_entries then begin
            let next = { data = Utop_zed_string.empty (); prev = node; size = 0 } in
            node.data <- entry;
            ...
            (size, count, node)
  in
  let rec node = { data = Utop_zed_string.empty (); size = 0; prev = node } in
...
let is_space ch = Utop_zed_char.for_all is_space_uChar ch
let is_empty str = Utop_zed_string.for_all is_space str
...
let drop_oldest history =
  ...
  last.data <- Utop_zed_string.empty ();
  last.size <- 0
    ...
    if ofs = len then
      ...
      match String.unsafe_get entry ofs with
        ...
        | ch when Char.code ch <= 127 ->
            ...
            loop (ofs + 1)
        | _ ->
            let ofs' = Utop_zed_utf8.unsafe_next entry ofs in
...
let escape entry =
  let len = Utop_zed_string.bytes entry in
  let buf = Utop_zed_string.Buf.create len in
  let zChar_n= Utop_zed_char.unsafe_of_char 'n' in
  let zChar_slash= Utop_zed_char.unsafe_of_char '\\' in
  let zChar_nl= Utop_zed_char.unsafe_of_char '\n' in
  let rec loop ofs =
    if ofs = len then
      Utop_zed_string.Buf.contents buf
    else
      let ch, ofs= Utop_zed_string.extract_next entry ofs in
      if Utop_zed_char.compare ch zChar_nl = 0 then
        begin
          Utop_zed_string.Buf.add_zChar buf zChar_slash;
          Utop_zed_string.Buf.add_zChar buf zChar_n;
          loop ofs;
        end
      else if Utop_zed_char.compare ch zChar_slash = 0 then
        begin
          Utop_zed_string.Buf.add_zChar buf zChar_slash;
          Utop_zed_string.Buf.add_zChar buf zChar_slash;
          loop ofs;
        ...
        begin
          Utop_zed_string.Buf.add_zChar buf ch;
          loop ofs;
        end
  in
  loop 0
    ...
    if ofs = len then
      ...
      match String.unsafe_get line ofs with
        ...
        | ch when Char.code ch <= 127 ->
            ...
            loop (ofs + 1) (size + 1)
        | _ ->
            let ofs' = Utop_zed_utf8.unsafe_next line ofs in
...
let unescape line =
  let eos= Utop_zed_string.bytes line in
  let buf= Utop_zed_string.Buf.create 0 in
  let zChar_n= Utop_zed_char.unsafe_of_char 'n' in
  let zChar_slash= Utop_zed_char.unsafe_of_char '\\' in
  let zChar_nl= Utop_zed_char.unsafe_of_char '\n' in
  let rec loop ofs size =
    if ofs >= eos then
      (Utop_zed_string.Buf.contents buf, size + 1)
    else
      let ch, ofs= Utop_zed_string.extract_next line ofs in
      if Utop_zed_char.compare ch zChar_slash = 0 then
        if ofs >= eos then
          (Utop_zed_string.Buf.add_zChar buf zChar_slash;
          (Utop_zed_string.Buf.contents buf, size + 3);)
        else
          (let next, ofs_next= Utop_zed_string.extract_next line ofs in
          if Utop_zed_char.compare next zChar_n = 0 then
            (Utop_zed_string.Buf.add_zChar buf zChar_nl;
            loop ofs_next (size + 2);)
          else if Utop_zed_char.compare next zChar_slash = 0 then
            (Utop_zed_string.Buf.add_zChar buf zChar_slash;
            loop ofs_next (size + 2);)
          else
            (Utop_zed_string.Buf.add_zChar buf zChar_slash;
        ...
        (Utop_zed_string.Buf.add_zChar buf ch;
        loop ofs (size + Utop_zed_char.size ch);)
  in
  loop 0 0
          ...
          Lwt.finalize (fun () ->
            let rec aux num =
              ...
              match line with
                ...
                | Some line ->
                  (try
                    let line= Utop_zed_string.of_utf8 line in
                       ...
                       if not (skip_empty && is_empty entry) && not (skip_dup && is_dup history entry) then begin
                        ...
                        Lwt.return ()
                    with
                      | Utop_zed_string.Invalid (msg, _)-> log num msg
                      | Utop_zed_utf8.Invalid (msg, _)-> log num msg
                   ) >>= fun () ->
                    aux (num + 1)
            in
            aux 1)
...
let rec dump_entries oc marker node =
  if node == marker then
    ...
    Lwt_io.write_line oc (Utop_zed_string.to_utf8 node.data) >>= fun () ->
          ...
          let rec aux count =
            ...
            match line with
              | None ->
                  ...
                  return count
              | Some line ->
                  let line= Utop_zed_string.unsafe_of_utf8 line in
                  ...
                  if not (skip_empty && is_empty line) && not (skip_dup && is_dup history_save line) then
                    add_aux history_save line (Utop_zed_string.bytes line + 1);
                  aux (count + 1)
          in
          aux 0
        end else
