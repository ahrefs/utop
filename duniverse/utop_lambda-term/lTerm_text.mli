...
(***)
val aval_width : Utop_zed_string.width -> int

(** {6 Conversions} *)

val of_string : Utop_zed_string.t -> t
  (** Creates a styled string from a string. All characters of the
      string have no style. *)

val to_string : t -> Utop_zed_string.t
...
val of_string_maybe_invalid : Utop_zed_string.t -> t
  (** Creates a styled string from a Utop_zed_string. All characters of the
...
val of_rope : Utop_zed_rope.t -> t
  (** Creates a styled string from a rope. *)

val to_rope : t -> Utop_zed_rope.t
...
val stylise_parenthesis : t -> ?paren : (Utop_zed_char.t * Utop_zed_char.t) list -> int -> LTerm_style.t -> unit
