...
module Query = struct
  ...
  *)
  ...
  let left n ctx= (* h *)
    let n= max 0 n in
    let edit= Utop_zed_edit.edit ctx in
    let lines= Utop_zed_edit.lines edit
    and line_idx= Utop_zed_edit.line ctx in
    let line_len= Utop_zed_lines.line_length lines line_idx in
    let column= Utop_zed_edit.column ctx in
    let dest= (column - n) |> max 0 |> min line_len in
    let positon= Utop_zed_edit.position ctx in
  ...
  let right ?(newline=false) n ctx= (* l *)
    ...
    let line_len=
      max 0 @@
      let len= Utop_zed_lines.line_length lines line_idx in
      if newline then len
      else len - 1
    in
    let column= Utop_zed_edit.column ctx in
  ...
  let line_FirstChar _n ctx= (* 0 *)
    ...
    let lines= Utop_zed_edit.lines edit
    and line_idx= Utop_zed_edit.line ctx in
    let start= Utop_zed_lines.line_start lines line_idx in
    let column= Utop_zed_edit.column ctx in
    (start, column - start)
  ...
  let line_LastChar ?(newline=false) n ctx= (* ^ *)
    ...
    let lines= Utop_zed_edit.lines edit
    and line_idx= Utop_zed_edit.line ctx in
    let count= Utop_zed_lines.count lines in
    let line_idx=
      if n > 1 then
        min count @@
        line_idx + (n - 1)
        ...
        line_idx
    in
    let stop= Utop_zed_lines.line_stop lines line_idx in
    if newline then
      stop
    else
      max 0 (stop - 1)
  ...
  let get_category ?(nl_as_sp=false) uchar=
    if uchar = Utop_zed_utf8.extract "\n" 0 && nl_as_sp then
      `Zs
    else
  ...
  let get_boundary multi_line ctx=
    let edit= Utop_zed_edit.edit ctx in
    if multi_line then
      (0, Utop_zed_rope.length (Utop_zed_edit.text edit))
    else
      let lines= Utop_zed_edit.lines edit
      and line_idx= Utop_zed_edit.line ctx in
      (Utop_zed_lines.line_start lines line_idx
      , Utop_zed_lines.line_stop lines line_idx)
  ...
  let next_category
    ...
    =
    let start_category=
      let zchar= Utop_zed_rope.get text pos in
      let core= Utop_zed_char.core zchar in
      get_category ~nl_as_sp core
    in
    let zip= Utop_zed_rope.Zip.make_f text pos in
    let rec skip_curr zip pos=
      if pos < stop then
        let zchar, zip= Utop_zed_rope.Zip.next zip in
        let category= get_category ~nl_as_sp (Utop_zed_char.core zchar) in
  ...
  let prev_category
    ...
    let rec skip_curr zip pos=
      if pos > start then
        let zchar, zip= Utop_zed_rope.Zip.prev zip in
        let category= get_category ~nl_as_sp (Utop_zed_char.core zchar) in
        if is_equal category start_category then
          skip_curr zip (pos - 1)
        else
  ...
  let goto_line ctx index=
    let edit= Utop_zed_edit.edit ctx in
    let lines= Utop_zed_edit.lines edit in
    let index= min index (Utop_zed_lines.count lines) in
    Utop_zed_lines.line_start lines index

  let next_line ctx delta=
    let edit= Utop_zed_edit.edit ctx in
    let text= Utop_zed_edit.text edit in
    let lines= Utop_zed_edit.lines edit in
    let index = Utop_zed_edit.line ctx in
    let cursor= Utop_zed_edit.cursor ctx in
    let count= Utop_zed_lines.count lines in
    if index = Utop_zed_lines.count lines then
      max 0 (Utop_zed_rope.length text - 1)
    else begin
      let stop =
        if index + delta >= count then
          Utop_zed_rope.length text
        else
          Utop_zed_lines.line_start lines (index + delta + 1) - 1
      in
      let wanted_idx= Utop_zed_lines.get_idx_by_width
          lines
          (min count (index + delta))
          (Utop_zed_cursor.get_wanted_column cursor)
      in
      max
        (Utop_zed_lines.line_start lines (index + delta))
  ...
  let prev_line ctx delta=
    ...
    let lines= Utop_zed_edit.lines edit in
    let cursor= Utop_zed_edit.cursor ctx in
    let index = Utop_zed_cursor.get_line cursor in
    if index - delta < 0 then
      0
    else
      let stop = Utop_zed_lines.line_start lines (index-delta+1) - 1 in
      let wanted_idx= Utop_zed_lines.get_idx_by_width
          lines
          (index - delta)
          (Utop_zed_cursor.get_wanted_column cursor)
  ...
  let next_word' ?(multi_line=true) ~next_category ~pos ~stop text=
    ...
    if next < stop then
      ...
      let zchar= Utop_zed_rope.get text next in
      let core= Utop_zed_char.core zchar in
  ...
  let line_FirstNonBlank _n ctx= (* ^ *)
    ...
    if line_len > 0 then
      if is_space (get_category (Utop_zed_char.core (Utop_zed_rope.get text start))) then
  ...
  let prev_word' ?(multi_line=true) ~prev_category ~pos ~start text=
    ...
    let start_category=
      ...
      get_category ~nl_as_sp core
    and before_start=
      let zchar= Utop_zed_rope.get text (pos - 1) in
      let core= Utop_zed_char.core zchar in
      get_category ~nl_as_sp core
    in
  ...
  let prev_word_end' ?(multi_line=true) ~prev_category ~pos ~start text=
    ...
    if is_space start_category then
      ...
      let before_category=
        let zchar= Utop_zed_rope.get text prev in
        let core= Utop_zed_char.core zchar in
        get_category ~nl_as_sp core
      in
  ...
  let occurrence_char ~pos ~stop chr text=
    try
      let zip= Utop_zed_rope.Zip.make_f text pos in
      let next= Utop_zed_rope.Zip.find_f
        (fun c-> Utop_zed_char.compare chr c = 0)
        zip
      in
      let next_pos= Utop_zed_rope.Zip.offset next in
  ...
  let occurrence_char_back ~pos ~start chr text=
    try
      let zip= Utop_zed_rope.Zip.make_f text pos in
      let prev= Utop_zed_rope.Zip.find_b
        (fun c-> Utop_zed_char.compare chr c = 0)
        zip
      in
      let prev_pos= Utop_zed_rope.Zip.offset prev in
  ...
  let occurrence ~pos ~stop ~cmp text=
    try
      let zip= Utop_zed_rope.Zip.make_f text pos in
      let next= Utop_zed_rope.Zip.find_f cmp zip in
      let next_pos= Utop_zed_rope.Zip.offset next in
      if next_pos < stop then
        Some (next_pos, Utop_zed_rope.get text next_pos)
      else
        None
    with _-> None
  ...
  let occurrence_back ~pos ~start ~cmp text=
    try
      let zip= Utop_zed_rope.Zip.make_f text pos in
      let prev= Utop_zed_rope.Zip.find_b cmp zip in
      let prev_pos= Utop_zed_rope.Zip.offset prev in
      if prev_pos > start then
        Some (prev_pos - 1, Utop_zed_rope.get text (prev_pos - 1))
      else
        None
    with _-> None
  ...
  let occurrence_pare ~pos ~level ~start ~stop pair text=
    let left, right= pair in
    let equal a b= Utop_zed_char.compare a b = 0 in
    ...
    if level > 0 && pos >= start && pos < stop then
      let init_pos=
        if equal (Utop_zed_rope.get text pos) left
        then pos+1
        else if equal (Utop_zed_rope.get text pos) right
        then pos
        else pos+1
      in
  ...
  let item_match ~start ~stop pos text=
    match Utop_zed_rope.get text pos |> Utop_zed_char.to_utf8 with
    | "("->
      (match occurrence_pare
        ~pos ~level:1 ~start ~stop
        (Utop_zed_char.of_utf8 "(", Utop_zed_char.of_utf8 ")") text
      with
  ...
  let include_word' ?(multi_line=true) ~next_category ~pos ~stop text=
    if Utop_zed_rope.length text = 0 then None else
...
module Vi = Utop_mew_vi.Core.Make (Concurrent)
include Vi

let of_lterm_code : LTerm_key.code -> Utop_mew_vi.Key.code= function
  | Char chr-> Char (Utop_zed_utf8.escaped_char chr)
  | Enter     -> Enter
  | Escape    -> Escape
  | Tab       -> Tab
  | Up        -> Up
...
let of_vi_code : Utop_mew_vi.Key.code -> LTerm_key.code= function
  | Char bin -> Char (Utop_zed_utf8.extract bin 0)
  | Enter     -> Enter
  | Escape    -> Escape
  | Tab       -> Tab
  | Up        -> Up
...
let of_lterm_key lterm_key=
  {
    Utop_mew_vi.Key.control= lterm_key.LTerm_key.control;
    meta= lterm_key.meta;
...
let of_vi_key vi_key=
  {
    LTerm_key.control= vi_key.Utop_mew_vi.Key.control;
    meta= vi_key.meta;
    shift= vi_key.shift;
...
let perform vi_edit ctx exec action=
  ...
  let delete ~register ?(line=false) ?boundary start len=
    let edit= Utop_zed_edit.edit ctx in
    let text= Utop_zed_edit.text edit in
    let eot= Utop_zed_rope.length text in
    let lines= Utop_zed_edit.lines edit in
    ...
    if len > 0 then
      let end_pos=
        if stop >= eot then
          ...
          if eot > 0 then
            if (=)
              (Utop_zed_char.core (Utop_zed_rope.get text end_pos))
              (Utop_zed_utf8.extract "\n" 0)
            then
              max 0 @@ end_pos - 1
            else
              end_pos
          ...
          if (=)
            (Utop_zed_char.core (Utop_zed_rope.get text stop))
            (Utop_zed_utf8.extract "\n" 0)
          then
            max
              (start - 1)
              Utop_zed_lines.(line_start lines (line_index lines start))
          else
            start
      in
      let content=
        let str= Utop_zed_rope.sub text start len
          |> Utop_zed_rope.to_string
          |> Utop_zed_string.to_utf8
        in
        if line then
      ...
      exec [
        Edit (Utop_zed (Utop_zed_edit.Goto start));
        Edit (Utop_zed (Utop_zed_edit.Kill_next_chars del_len));
        Edit (Utop_zed (Utop_zed_edit.Goto end_pos))
        ]
    else
      return (ContinueLoop [])
  in
  let change ~register ?(line=false) ?boundary start len=
    ...
    if len > 0 then
      ...
      exec [
        Edit (Utop_zed (Utop_zed_edit.Goto start));
        Edit (Utop_zed (Utop_zed_edit.Kill_next_chars len));
        Edit (Utop_zed (Utop_zed_edit.Goto start))
        ]
    else
      return (ContinueLoop [])
  ...
  let yank ~register ?(line=false) start len=
    ...
    vi_edit#set_register register content;
    vi_edit#set_register "\"" content;
    Utop_zed_edit.copy_sequence ctx start len;
    return (ContinueLoop [])
  in
  let setup_pos ()=
    let edit= Utop_zed_edit.edit ctx in
    let text= Utop_zed_edit.text edit in
    let pos= Utop_zed_edit.position ctx in
    let text_len= Utop_zed_rope.length text in
    (if text_len > 0 then
      let step= if pos >= text_len then pos - 1 else pos in
      let step=
        if (=)
          (Utop_zed_char.core (Utop_zed_rope.get text step))
          (Utop_zed_utf8.extract "\n" 0)
        then max 0 @@ step - 1
        else step
      in
      exec [Edit (Utop_zed (Utop_zed_edit.Goto step))]
    else
      exec [Edit (Utop_zed (Utop_zed_edit.Goto_bol))])
  in
  let pare_include pair level action=
    let text= Utop_zed_edit.text (Utop_zed_edit.edit ctx) in
    let pos= Utop_zed_edit.position ctx in
    let start= 0
    and stop= Utop_zed_rope.length text in
  ...
  | Vi_action.Insert (insert, count)->
    ...
    | Newline_below _s->
      exec @@
        (Edit (Utop_zed (Utop_zed_edit.Goto_eol)))::
        (list_make (Edit (Utop_zed (Utop_zed_edit.Newline))) count)
    | Newline_above _s->
      exec @@
        list_dup [
          Edit (Utop_zed (Utop_zed_edit.Goto_bol));
          Edit (Utop_zed (Utop_zed_edit.Newline));
          Edit (Utop_zed (Utop_zed_edit.Prev_line));
        ]
        count
  ...
  | Motion (motion, count)->
    ...
    | Left->
      let rec left n=
        if n > 0 then
          ...
          exec
            (list_make
              (Edit (Utop_zed (Utop_zed_edit.Goto pos))) 1) >>=
          (function
    ...
    | Upward->
      ...
      exec [Edit (Utop_zed (Utop_zed_edit.Set_pos pos))]
    | Downward->
    ...
    | Word->
      ...
      let rec next_word n=
        ...
        if n > 0 && pos < stop then
          ...
          in
          exec
            [Edit (Utop_zed (Utop_zed_edit.Goto next))]
          >>= (function
            | Result _ as r-> return r
    ...
    | Word_back->
      ...
      let rec prev_word n=
        let pos= min (stop - 1) (Utop_zed_edit.position ctx) in
        if n > 0 && pos > start then
          ...
          exec
            (list_make
              (Edit (Utop_zed (Utop_zed_edit.Goto prev))) 1) >>=
          (function
            | Result _ as r-> return r
    ...
    | WORD_back->
      ...
      let rec prev_word n=
        ...
        if n > 0 && pos > start then
          ...
          in
          exec
            [Edit (Utop_zed (Utop_zed_edit.Goto prev))]
          >>= (function
            | Result _ as r-> return r
            | ContinueLoop _-> prev_word (n-1))
        else
    ...
    | Line_FirstNonBlank->
      ...
      exec
        [Edit (Utop_zed (Utop_zed_edit.Goto nonblank))]
    | Line_LastChar->
      let rec lastChar n=
        if n > 0 then
          let pos= Query.line_LastChar n ctx in
          exec
            [Edit (Utop_zed (Utop_zed_edit.Goto pos))]
          >>= (function
            | Result _ as r-> return r
    ...
    | GotoLine->
      ...
      exec [Edit (Utop_zed (Utop_zed_edit.Set_pos pos))]
    | GotoLine_first->
      exec [Edit (Utop_zed (Utop_zed_edit.Goto_bot))]
    | GotoLine_last->
      exec [
        Edit (Utop_zed (Utop_zed_edit.Goto_eot));
        Edit (Utop_zed (Utop_zed_edit.Prev_char))
        ]
    | Occurrence_inline chr->
      let edit= Utop_zed_edit.edit ctx in
      let text= Utop_zed_edit.text edit in
      let pos= Utop_zed_edit.position ctx + 1 in
      let lines= Utop_zed_edit.lines edit in
      let line= Utop_zed_edit.line ctx in
      let stop= Utop_zed_lines.line_stop lines line in
      let rec query_n chr pos n=
        ...
        if n = 1 then next else
          ...
          | None-> None
      in
      (match query_n (Utop_zed_char.of_utf8 chr) pos count with
      | Some pos->
        exec [ Edit (Utop_zed (Utop_zed_edit.Goto pos)) ]
      | None-> return (ContinueLoop []))
    | Occurrence_inline_back chr->
      ...
      let lines= Utop_zed_edit.lines edit in
      let line= Utop_zed_edit.line ctx in
      let start= Utop_zed_lines.line_start lines line in
      let rec query_n chr pos n=
        if n < 1 then None else
        ...
        if n = 1 then prev else
          ...
          | None-> None
      in
      (match query_n (Utop_zed_char.of_utf8 chr) (pos-1) count with
      | Some pos->
        exec [ Edit (Utop_zed (Utop_zed_edit.Goto pos)) ]
      | None-> return (ContinueLoop []))
    | Occurrence_inline_till chr->
      ...
      (match query_n (Utop_zed_char.of_utf8 chr) pos count with
      | Some pos->
        exec [ Edit (Utop_zed (Utop_zed_edit.Goto (pos-1))) ]
      | None-> return (ContinueLoop []))
    ...
    | Match->
      ...
      let text= Utop_zed_edit.text edit in
      let pos= Utop_zed_edit.position ctx in
      let stop= Utop_zed_rope.length text in
  ...
  | Delete (register, motion, count)->
    ...
    | Right_nl->
      ...
      exec [
        Edit (Utop_zed (Utop_zed_edit.Goto pos));
        Edit (Utop_zed (Utop_zed_edit.Kill_next_chars delta));
        ]
    | Upward->
      let edit= Utop_zed_edit.edit ctx in
      let lines= Utop_zed_edit.lines edit in
      ...
      if line_delta > 0 then
        let pos_start= Utop_zed_lines.line_start lines dest
        and pos_end= Utop_zed_lines.line_stop lines line in
    ...
    | Downward->
      let edit= Utop_zed_edit.edit ctx in
      let lines= Utop_zed_edit.lines edit in
      let line_count= Utop_zed_lines.count lines in
      let line= Utop_zed_edit.line ctx in
      let dest= min line_count (line + count) in
      let line_delta = dest - line in
      if line_delta > 0 then
        let pos_start= Utop_zed_lines.line_start lines line
        and pos_end= Utop_zed_lines.line_stop lines dest in
        let pos_end=
          if dest < line_count
          then pos_end + 1
          else pos_end in
    ...
    | Word_back_end->
      let edit= Utop_zed_edit.edit ctx in
      let text= Utop_zed_edit.text edit in
      if Utop_zed_rope.length text <= 0 then return (ContinueLoop []) else
      let start, stop= Query.get_boundary true ctx in
      let pos= min (stop - 1) (Utop_zed_edit.position ctx) in
      let rec prev_word pos n=
        if n > 0 && pos > start then
    ...
    | Line_LastChar_nl->
      ...
      delete pos (next+1 - pos)
    | Parenthesis_include->
      pare_include Utop_zed_char.(of_utf8 "(", of_utf8 ")") count delete
    | Parenthesis_inner->
      pare_inner Utop_zed_char.(of_utf8 "(", of_utf8 ")") count delete
    | Bracket_include->
      pare_include Utop_zed_char.(of_utf8 "[", of_utf8 "]") count delete
    | Bracket_inner->
      pare_inner Utop_zed_char.(of_utf8 "[", of_utf8 "]") count delete
    ...
    | Occurrence_inline chr->
      ...
      (match query_n (Utop_zed_char.of_utf8 chr) pos count with
      | Some pos->
        let start= Utop_zed_edit.position ctx in
    ...
    | Occurrence_inline_back chr->
      ...
      (match query_n (Utop_zed_char.of_utf8 chr) (pos-1) count with
      | Some pos->
        let stop= Utop_zed_edit.position ctx in
    ...
    | WORD_inner->
      ...
      | None-> return (ContinueLoop []))
    | Quote_inner chr->
      let quote= Utop_zed_char.of_utf8 chr in
  ...
  | Change (register, motion, count)->
    ...
    | Line_LastChar_nl->
      ...
      change pos (next+1 - pos)
    | Parenthesis_include->
      pare_include Utop_zed_char.(of_utf8 "(", of_utf8 ")") count change
    | Parenthesis_inner->
      pare_inner Utop_zed_char.(of_utf8 "(", of_utf8 ")") count change
    | Bracket_include->
      pare_include Utop_zed_char.(of_utf8 "[", of_utf8 "]") count change
    | Bracket_inner->
      pare_inner Utop_zed_char.(of_utf8 "[", of_utf8 "]") count change
  ...
  | Yank (register, motion, count)->
    ...
    | Line_LastChar_nl->
      ...
      yank pos (next+1 - pos)
    | Parenthesis_include->
      pare_include Utop_zed_char.(of_utf8 "(", of_utf8 ")") count yank
    | Parenthesis_inner->
      pare_inner Utop_zed_char.(of_utf8 "(", of_utf8 ")") count yank
    | Bracket_include->
      pare_include Utop_zed_char.(of_utf8 "[", of_utf8 "]") count yank
    | Bracket_inner->
      pare_inner Utop_zed_char.(of_utf8 "[", of_utf8 "]") count yank
    ...
    | Word_include->
      ...
      (match move_n pos count with
      | Some (word_begin, word_end)->
        Utop_zed_edit.copy_sequence ctx word_begin (word_end+1 - word_begin);
        return (ContinueLoop [])
      | None-> return (ContinueLoop []))
    | WORD_include->
      let edit= Utop_zed_edit.edit ctx in
  ...
  | Undo count->
    exec @@ list_dup [
      Edit (Utop_zed (Utop_zed_edit.Undo));
      ] count
  ...
  | Paste_before (register, count)->
    let action_paste=
      ...
      | Some (Seq str)-> [
        Edit (Utop_zed (Utop_zed_edit.Insert_str (Utop_zed_string.of_utf8 str)));
        Edit (Utop_zed (Utop_zed_edit.Prev_char));
        ]
      | Some (Line str)-> [
        Edit (Utop_zed (Utop_zed_edit.Goto_bol));
  ...
  | Paste_after (register, count)->
    let action_paste=
      ...
      | Some (Seq str)->
        ...
        in
        if Utop_zed_edit.at_eol ctx then
          actions
        else
          Edit (Utop_zed (Utop_zed_edit.Next_char)) :: actions
      | Some (Line str)->
        [
        Edit (Utop_zed (Utop_zed_edit.Goto_eol));
        Edit (Utop_zed (Utop_zed_edit.Insert_str
          (Utop_zed_string.of_utf8
  ...
  | Join count->
    exec @@
      (list_make (Edit (Utop_zed (Utop_zed_edit.Join_line))) count)
  | DeleteSelected register->
    let delete= delete ~register in
    let edit= Utop_zed_edit.edit ctx in
    if Utop_zed_edit.get_selection edit then
      let a = Utop_zed_edit.position ctx and b = Utop_zed_cursor.get_position (Utop_zed_edit.mark edit) in
  ...
  | ChangeMode mode->
    let edit= Utop_zed_edit.edit ctx in
    (match mode with
    | Insert-> Utop_zed_edit.set_selection edit false
    | Normal-> Utop_zed_edit.set_selection edit false
    | Visual-> Utop_zed_edit.set_mark ctx
    | Commandline-> Utop_zed_edit.set_selection edit false);
    return (ContinueLoop [])
