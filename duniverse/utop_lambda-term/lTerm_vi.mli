...
module Query :
  sig
    val left : int -> 'a Utop_zed_edit.context -> int * int
    val right : ?newline:bool -> int -> 'a Utop_zed_edit.context -> int * int
    val line_FirstChar : 'a -> 'b Utop_zed_edit.context -> int * int
    val line_LastChar : ?newline:bool -> int -> 'a Utop_zed_edit.context -> int
    val get_category :
      ?nl_as_sp:bool ->
      Uchar.t ->
      Uucp.Gc.t
    val get_boundary : bool -> 'a Utop_zed_edit.context -> int * int
    ...
    val next_category :
      ...
      pos:int -> stop:int -> Utop_zed_rope.t -> int
    val prev_category :
      ?nl_as_sp:bool ->
      ?is_equal:(Uucp.Gc.t -> Uucp.Gc.t -> bool) ->
      pos:int -> start:int -> Utop_zed_rope.t -> int
    val goto_line : 'a Utop_zed_edit.context -> int -> int
    val next_line : 'a Utop_zed_edit.context -> int -> int
    val prev_line : 'a Utop_zed_edit.context -> int -> int
    val next_word' :
      ?multi_line:bool ->
      next_category:(nl_as_sp:bool -> pos:int -> stop:int -> Utop_zed_rope.t -> int) ->
      pos:int -> stop:int -> Utop_zed_rope.t -> int
    val next_word :
      ?multi_line:bool -> pos:int -> stop:int -> Utop_zed_rope.t -> int
    val next_WORD :
      ?multi_line:bool -> pos:int -> stop:int -> Utop_zed_rope.t -> int
    val line_FirstNonBlank : 'a -> 'b Utop_zed_edit.context -> int
    val prev_word' :
      ?multi_line:bool ->
      prev_category:(nl_as_sp:bool -> pos:int -> start:int -> Utop_zed_rope.t -> int) ->
      pos:int -> start:int -> Utop_zed_rope.t -> int
    val prev_word :
      ?multi_line:bool -> pos:int -> start:int -> Utop_zed_rope.t -> int
    val prev_WORD :
      ?multi_line:bool -> pos:int -> start:int -> Utop_zed_rope.t -> int
    val next_word_end' :
      ?multi_line:bool ->
    ...
    val prev_WORD_end :
      ?multi_line:bool -> pos:int -> start:int -> Utop_zed_rope.t -> int
    val occurrence_char :
      pos:int -> stop:int -> Utop_zed_char.t -> Utop_zed_rope.t -> int option
    val occurrence_char_back :
      pos:int -> start:int -> Utop_zed_char.t -> Utop_zed_rope.t -> int option
    val occurrence :
      pos:int ->
      stop:int ->
      cmp:(Utop_zed_char.t -> bool) -> Utop_zed_rope.t -> (int * Utop_zed_char.t) option
    val occurrence_back :
      pos:int ->
      start:int ->
      cmp:(Utop_zed_char.t -> bool) -> Utop_zed_rope.t -> (int * Utop_zed_char.t) option
    val occurrence_pare_raw :
      ...
      level:int ->
      start:int ->
      stop:int -> Utop_zed_char.t * Utop_zed_char.t -> Utop_zed_rope.t -> (int * int) option
    val occurrence_pare :
      pos:int ->
      level:int ->
      start:int ->
      stop:int -> Utop_zed_char.t * Utop_zed_char.t -> Utop_zed_rope.t -> (int * int) option
    val item_match : start:int -> stop:int -> int -> Utop_zed_rope.t -> int option
    val include_word' :
      ?multi_line:bool ->
      next_category:(nl_as_sp:bool -> pos:int -> stop:int -> Utop_zed_rope.t -> int) ->
      pos:int -> stop:int -> Utop_zed_rope.t -> (int * int) option
    val include_word :
      ?multi_line:bool ->
      pos:int -> stop:int -> Utop_zed_rope.t -> (int * int) option
    val include_WORD :
    ...
    val inner_word' :
      ?multi_line:bool ->
      prev_category:(nl_as_sp:bool -> pos:int -> start:int -> Utop_zed_rope.t -> int) ->
      next_category:(nl_as_sp:bool -> pos:int -> stop:'a -> Utop_zed_rope.t -> int) ->
      pos:int -> stop:'a -> Utop_zed_rope.t -> (int * int) option
    val inner_word :
      ?multi_line:bool ->
      pos:int -> stop:int -> Utop_zed_rope.t -> (int * int) option
    val inner_WORD :
...
module Vi :
  sig
    module Edit_action = Utop_mew_vi.Edit_action
    module Vi_action = Utop_mew_vi.Vi_action
    module Base :
      sig
        module Key :
          sig
            type t = Utop_mew_vi.Modal.Key.t
            type code = Utop_mew_vi.Modal.Key.code
            type modifier = Utop_mew_vi.Modal.Key.modifier
            type modifiers = Utop_mew_vi.Modal.Key.modifiers
        ...
        module Mode :
          sig
            ...
            type name = Utop_mew_vi.Modal.Name.t
            type action =
              |  Switch of name
              | Key of Key.t
        ...
        class state :
          Mode.t Utop_mew_vi.Modal.Mode.Modes.t ->
          object
            val mutable default_mode : Utop_mew_vi.Modal.Mode.Modes.key * Mode.t
            val mutable timeout : float
            method default_mode : Utop_mew_vi.Modal.Mode.Modes.key * Mode.t
            method edit : edit
      ...
      sig
        ...
        module Resolver :
          sig
            ...
            and config= {
              mode: Utop_mew_vi.Mode.Name.t Utop_react.signal;
              set_mode: ?step:Utop_react.step -> Utop_mew_vi.Mode.Name.t -> unit;
              keyseq: keyseq Utop_react.signal;
              set_keyseq: ?step:Utop_react.step -> keyseq -> unit;
            ...
            and result=
              | Accept of (Edit_action.t * keyseq * Utop_mew_vi.Mode.Name.t)
            ...
            module Common :
              sig
                val try_count : t -> t
                val try_motion : Utop_mew_vi.Mode.name -> t
              end
            module Normal :
              sig
...
class edit : state ->
  object
    ...
    val i : Utop_mew_vi.Key.t Lwt_mvar.t
    val o : Utop_mew_vi.Key.t Lwt_mvar.t
    ...
    method i : Utop_mew_vi.Key.t Lwt_mvar.t
    method keyin : Utop_mew_vi.Key.t -> unit Lwt.t
    method o : Utop_mew_vi.Key.t Lwt_mvar.t
...
val of_lterm_code : LTerm_key.code -> Utop_mew_vi.Key.code
val of_vi_code : Utop_mew_vi.Key.code -> LTerm_key.code
val of_lterm_key : LTerm_key.t -> Utop_mew_vi.Key.t
val of_vi_key : Utop_mew_vi.Key.t -> LTerm_key.t

open LTerm_read_line_base

val perform :
  edit ->
  'a Utop_zed_edit.context ->
