...
val zed_common_prefix : Utop_zed_string.t list -> Utop_zed_string.t
  (** Returns the common prefix of a list of words. *)

val lookup : Utop_zed_utf8.t -> Utop_zed_utf8.t list -> Utop_zed_utf8.t list
  (** [lookup word words] lookup for completion of [word] into
      [words]. It returns all words starting with [word]. *)

val lookup_assoc : Utop_zed_utf8.t -> (Utop_zed_utf8.t * 'a) list -> (Utop_zed_utf8.t * 'a) list
...
val bindings : action list Utop_zed_input.Make(LTerm_key).t ref
  (** Bindings. *)
...
val macro : action Utop_zed_macro.t
...
(** The read-line engine. If no clipboard is provided,
    ...
    {!macro} is used. *)
class virtual ['a] engine : ?history : history -> ?clipboard : Utop_zed_edit.clipboard -> ?macro : action Utop_zed_macro.t -> unit -> object
  ...
  method edit : unit Utop_zed_edit.t
    (** The edition engine used by this read-line engine. *)

  method context : unit Utop_zed_edit.context
    (** The context for the edition engine. *)

  method clipboard : Utop_zed_edit.clipboard
    (** The clipboard used by the edition engine. *)

  method macro : action Utop_zed_macro.t
    (** The macro recorder. *)

  method input_prev : Utop_zed_rope.t
    (** The input before the cursor. *)

  method input_next : Utop_zed_rope.t
  ...
  method history : (Utop_zed_string.t list * Utop_zed_string.t list) signal
    (** The history zipper. *)
  ...
  method completion_words : (Utop_zed_string.t * Utop_zed_string.t) list signal
  ...
  method set_completion : ?index:int -> int -> (Utop_zed_string.t * Utop_zed_string.t) list -> unit
...
class read_line : ?history : history -> unit -> object
  inherit [Utop_zed_string.t] engine

  method eval : Utop_zed_string.t
...
class ['a] read_keyword : ?history : history -> unit -> object
  ...
  method keywords : (Utop_zed_string.t * 'a) list
...
class virtual ['a] term : LTerm.t -> object
  ...
  method private exec : ?keys : LTerm_key.t list -> action list -> 'a loop_result Lwt.t
    (** Executes a list of actions. Rememver to call [Utop_zed_macro.add
