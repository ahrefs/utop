...
module Make (LiteralIntf: LiteralIntf.Type) = struct
  ...
  class button ?brackets initial_label =
    let (bl, br)=
      ...
      | None-> Utop_zed_string.unsafe_of_utf8 "< ",Utop_zed_string.unsafe_of_utf8 " >"
    in
    let brackets_size = LTerm_text.aval_width (Utop_zed_string.width bl)
      + LTerm_text.aval_width (Utop_zed_string.width br)
    in
    ...
    val mutable size_request = { rows = 1; cols = brackets_size + (LTerm_text.aval_width (Utop_zed_string.width (LiteralIntf.to_string_exn initial_label))) }
    ...
    method set_label text =
      ...
      label <- text;
      size_request <- { rows = 1; cols = brackets_size + (LTerm_text.aval_width (Utop_zed_string.width text)) };
      self#queue_draw
    ...
    method! draw ctx focused =
      ...
      let width = LTerm_text.aval_width (Utop_zed_string.width label) in
      self#apply_style ctx focused;
      LTerm_draw.draw_string ctx (rows / 2) ((cols - width - brackets_size) / 2)
        (Utop_zed_string.append (Utop_zed_string.append bl label) br)
  end
  ...
  class checkbutton initial_label initial_state = object(self)
    ...
    method! draw ctx focused =
      ...
      let checked = Utop_zed_string.unsafe_of_utf8 (if state then "[x] " else "[ ] ") in
      self#apply_style ctx focused;
      LTerm_draw.draw_string ctx (rows / 2) 0 (Utop_zed_string.append checked label);
  ...
  class ['a] radiobutton (group : 'a radiogroup) initial_label (id : 'a) = object(self)
    ...
    method! draw ctx focused =
      let { rows; _ } = LTerm_draw.size ctx in
      let checked = Utop_zed_string.unsafe_of_utf8 (if state then "(o) " else "( ) ") in
      self#apply_style ctx focused;
      LTerm_draw.draw_string ctx (rows / 2) 0 (Utop_zed_string.append checked self#label_zed);
