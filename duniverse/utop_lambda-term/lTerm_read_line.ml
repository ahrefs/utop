...
type history = Utop_zed_string.t list
...
let common_prefix_one a b =
  let rec loop ofs =
    if ofs = String.length a || ofs = String.length b then
      ...
      let ch1, ofs1 = Utop_zed_utf8.unsafe_extract_next a ofs
      and ch2, ofs2 = Utop_zed_utf8.unsafe_extract_next b ofs in
...
let zed_common_prefix_one a b =
  let rec loop ofs =
    if ofs = Utop_zed_string.bytes a || ofs = Utop_zed_string.bytes b then
      Utop_zed_string.sub_ofs ~ofs:0 ~len:ofs a
    else
      let ch1, ofs1= Utop_zed_string.extract_next a ofs
      and ch2, ofs2= Utop_zed_string.extract_next b ofs in
      if ch1 = ch2 && ofs1 = ofs2 then
        loop ofs1
      else
        Utop_zed_string.sub_ofs ~ofs:0 ~len:ofs a
...
let zed_common_prefix = function
  | [] -> Utop_zed_string.empty ()
  | word :: rest -> List.fold_left zed_common_prefix_one word rest

let lookup word words = List.filter (fun word' -> Utop_zed_utf8.starts_with word' word) words
let lookup_assoc word words = List.filter (fun (word', _) -> Utop_zed_utf8.starts_with word' word) words
...
let () =
  bind [{ control = false; meta = false; shift = false; code = Home }] [Edit (LTerm_edit.Utop_zed Utop_zed_edit.Goto_bot)];
  bind [{ control = false; meta = false; shift = false; code = End }] [Edit (LTerm_edit.Utop_zed Utop_zed_edit.Goto_eot)];
  ...
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'b') }] [Edit (LTerm_edit.Utop_zed Utop_zed_edit.Prev_char)];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'f') }] [Edit (LTerm_edit.Utop_zed Utop_zed_edit.Next_char)];
  bind [{ control = true; meta = false; shift = false; code = Char(Uchar.of_char 'h') }] [Edit (LTerm_edit.Utop_zed Utop_zed_edit.Delete_prev_char)];
  ...
  bind [{ control = false; meta = true; shift = false; code = Enter }] [Edit (LTerm_edit.Utop_zed Utop_zed_edit.Newline)];
...
let is_prefix ~prefix s=
  let prefix= Utop_zed_string.to_utf8 prefix
  and s= Utop_zed_string.to_utf8 s in
...
let search_string str sub =
  let str= Utop_zed_string.to_utf8 str
  and sub= Utop_zed_string.to_utf8 sub in
  ...
  let rec loop ofs idx =
    if ofs + String.length sub > String.length str then
      ...
      if equal_at ofs 0 then
        ...
        loop (Utop_zed_utf8.unsafe_next str ofs) (idx + 1)
  in
  loop 0 0

let macro = Utop_zed_macro.create []
...
type completion_state = {
  ...
  words : (Utop_zed_string.t * Utop_zed_string.t) list;
}
...
type search_status =
  { before : Utop_zed_string.t list
  ; after  : Utop_zed_string.t list
  ; match_ : (Utop_zed_string.t * int) option
  }

class virtual ['a] engine ?(history = []) ?(clipboard = LTerm_edit.clipboard) ?(macro = macro) () =
  let edit : unit Utop_zed_edit.t = Utop_zed_edit.create ~clipboard () in
  let context = Utop_zed_edit.context edit (Utop_zed_edit.new_cursor edit) in
  ...
  let reset_completion_state =
    E.when_
      ...
      (E.select [
         E.stamp (Utop_zed_edit.changes edit                                    ) no_completion;
         E.stamp (S.changes (Utop_zed_cursor.position (Utop_zed_edit.cursor context))) no_completion;
       ])
  in
  ...
  let history_prefix, set_history_prefix =
    ...
    let edit_changes = Utop_zed_edit.changes edit in
    let edit_changes = E.map (fun _ -> Utop_zed_edit.text edit) edit_changes in
    let prefix = S.hold (Utop_zed_rope.empty ()) (E.select [ev; edit_changes]) in
    prefix, send
  in
object(self)
  ...
  val mutable save = (0, Utop_zed_rope.empty ())
  ...
  method input_prev =
    Utop_zed_rope.before (Utop_zed_edit.text edit) (Utop_zed_edit.position context)

  method input_next =
    Utop_zed_rope.after (Utop_zed_edit.text edit) (Utop_zed_edit.position context)
  ...
  method complete =
    ...
    let prefix_length = Utop_zed_edit.position context - comp.start in
    match comp.words with
    | [] -> ()
    | [(completion, suffix)] ->
      Utop_zed_edit.insert context (Utop_zed_rope.of_string
        (Utop_zed_string.sub completion ~pos:prefix_length
          ~len:(Utop_zed_string.length completion - prefix_length)));
      Utop_zed_edit.insert context (Utop_zed_rope.of_string suffix)
    | (completion, _suffix) :: rest ->
      ...
      in
      Utop_zed_edit.insert context (Utop_zed_rope.of_string
        (Utop_zed_string.sub word ~pos:prefix_length
          ~len:(Utop_zed_string.length word - prefix_length)))
  ...
  method private search direction =
    let do_search direction =
      ...
      in
      let input = Utop_zed_rope.to_string (Utop_zed_edit.text edit) in
        ...
        | entry :: rest ->
          ...
          | Some pos -> begin
              ...
              | _ ->
                ...
                for i = pos to pos + Utop_zed_rope.length (Utop_zed_edit.text edit) - 1 do
    ...
    | Edition ->
      let text = Utop_zed_edit.text edit in
      Utop_zed_edit.goto context 0;
      Utop_zed_edit.remove context (Utop_zed_rope.length text);
      let prev, next = S.value history in
      set_history (Utop_zed_rope.to_string text :: (List.rev_append next prev), []);
      search_status <- None;
      set_mode Search;
      do_search direction
    | _ ->
  ...
  method insert ch =
    Utop_zed_edit.insert_char context ch

  method send_action action =
    if action <> Edit LTerm_edit.Stop_macro then Utop_zed_macro.add macro action;
    match action with
      | (Complete | Complete_bar | Accept) when S.value mode = Search -> begin
          ...
          match search_status with
            | Some { match_ = Some (entry, _pos); _ } ->
                search_status <- None;
                Utop_zed_edit.goto context 0;
                Utop_zed_edit.remove context (Utop_zed_rope.length (Utop_zed_edit.text edit));
                Utop_zed_edit.insert context (Utop_zed_rope.of_string entry)
            | Some { match_ = None; _ } | None ->
                ()
        end

      | Edit (LTerm_edit.Utop_zed action) ->
          Utop_zed_edit.get_action action context

      | Interrupt_or_delete_next_char ->
          if Utop_zed_rope.is_empty (Utop_zed_edit.text edit) then
            Lwt.async (fun ()-> Lwt_mvar.put interrupt Interrupt)
          else
            Utop_zed_edit.delete_next_char context
      ...
      | Complete_bar when S.value mode = Edition ->
          ...
          if comp.words <> [] then begin
            ...
            Utop_zed_edit.insert context (Utop_zed_rope.of_string
              (Utop_zed_string.after completion prefix_length));
            Utop_zed_edit.insert context (Utop_zed_rope.of_string suffix)
          end
      ...
      | History_prev when S.value mode = Edition ->begin
          ...
          match prev with
            ...
            | line :: rest ->
                let text = Utop_zed_edit.text edit in
                set_history (rest, Utop_zed_rope.to_string text :: next);
                Utop_zed_edit.goto context 0;
                Utop_zed_edit.remove context (Utop_zed_rope.length text);
                Utop_zed_edit.insert context (Utop_zed_rope.of_string line)
        end
      ...
      | History_next when S.value mode = Edition -> begin
          ...
          match next with
            ...
            | line :: rest ->
                let text = Utop_zed_edit.text edit in
                set_history (Utop_zed_rope.to_string text :: prev, rest);
                Utop_zed_edit.goto context 0;
                Utop_zed_edit.remove context (Utop_zed_rope.length text);
                Utop_zed_edit.insert context (Utop_zed_rope.of_string line)
        end
      ...
      | History_search_prev when S.value mode = Edition -> begin
          let prev, next = S.value history in
          let text = Utop_zed_rope.to_string @@ Utop_zed_edit.text edit in
          let prefix = S.value history_prefix in
          match history_find (is_prefix ~prefix:(Utop_zed_rope.to_string prefix)) prev with
          | None ->
            ()
          | Some (not_matched, line, rest) ->
            set_history (rest, not_matched @ text :: next);
            Utop_zed_edit.goto context 0;
            Utop_zed_edit.delete_next_line context;
            Utop_zed_edit.insert context (Utop_zed_rope.of_string line);
      ...
      | History_search_next when S.value mode = Edition -> begin
          let prev, next = S.value history in
          let prefix = S.value history_prefix in
          match history_find (is_prefix ~prefix:(Utop_zed_rope.to_string prefix)) next with
          | None ->
            ()
          | Some (not_matched, line, rest) ->
            let text = Utop_zed_rope.to_string @@ Utop_zed_edit.text edit in
      ...
      | Edit LTerm_edit.Start_macro when S.value mode = Edition ->
          Utop_zed_macro.set_recording macro true

      | Edit LTerm_edit.Stop_macro ->
          Utop_zed_macro.set_recording macro false

      | Edit LTerm_edit.Cancel_macro ->
          Utop_zed_macro.cancel macro

      | Edit LTerm_edit.Play_macro ->
          Utop_zed_macro.cancel macro;
          List.iter self#send_action (Utop_zed_macro.contents macro)

      | Edit LTerm_edit.Insert_macro_counter ->
          Utop_zed_edit.insert context (Utop_zed_rope.of_string (Utop_zed_string.unsafe_of_utf8 (string_of_int (Utop_zed_macro.get_counter macro))));
          Utop_zed_macro.add_counter macro 1

      | Edit LTerm_edit.Set_macro_counter when S.value mode = Edition ->
          let text = Utop_zed_edit.text edit in
          save <- (Utop_zed_edit.position context, text);
          Utop_zed_edit.goto context 0;
          Utop_zed_edit.remove context (Utop_zed_rope.length text);
      ...
      | Accept -> begin
          match S.value mode with
            ...
            | Set_counter ->
                ...
                save <- (0, Utop_zed_rope.empty ());
                (try
                   Utop_zed_macro.set_counter macro (int_of_string (Utop_zed_string.to_utf8 (Utop_zed_rope.to_string (Utop_zed_edit.text edit))))
                 with Failure _ ->
                   ());
                Utop_zed_edit.goto context 0;
                Utop_zed_edit.remove context (Utop_zed_rope.length (Utop_zed_edit.text edit));
                Utop_zed_edit.insert context text;
                Utop_zed_edit.goto context pos;
                set_mode Edition;
                set_message None
            | Add_counter ->
                let pos, text = save in
                save <- (0, Utop_zed_rope.empty ());
                (try
                   Utop_zed_macro.add_counter macro (int_of_string (Utop_zed_string.to_utf8 (Utop_zed_rope.to_string (Utop_zed_edit.text edit))))
                 with Failure _ ->
                   ());
                Utop_zed_edit.goto context 0;
                Utop_zed_edit.remove context (Utop_zed_rope.length (Utop_zed_edit.text edit));
  ...
  method stylise last =
    let txt = LTerm_text.of_rope (Utop_zed_edit.text edit) in
    let pos = Utop_zed_edit.position context in
    if not last && Utop_zed_edit.get_selection edit then begin
      let mark = Utop_zed_cursor.get_position (Utop_zed_edit.mark edit) in
...
class virtual ['a] abstract = object
  ...
  method virtual edit : unit Utop_zed_edit.t
  method virtual context : unit Utop_zed_edit.context
  method virtual clipboard : Utop_zed_edit.clipboard
  method virtual macro : action Utop_zed_macro.t
  method virtual stylise : bool -> LTerm_text.t * int
  method virtual history : (Utop_zed_string.t list * Utop_zed_string.t list) signal
  method virtual message : LTerm_text.t option signal
  method virtual input_prev : Utop_zed_rope.t
  method virtual input_next : Utop_zed_rope.t
  method virtual completion_words : (Utop_zed_string.t * Utop_zed_string.t) list signal
  method virtual completion_index : int signal
  method virtual set_completion : ?index:int -> int -> (Utop_zed_string.t * Utop_zed_string.t) list -> unit
...
class read_line ?history () = object(self)
  inherit [Utop_zed_string.t] engine ?history ()
  method eval = Utop_zed_rope.to_string (Utop_zed_edit.text self#edit)
end

class read_password () = object(self)
  inherit [Utop_zed_string.t] engine () as super

  method! stylise last =
    let text, pos = super#stylise last in
    for i = 0 to Array.length text - 1 do
      let _ch, style = text.(i) in
      text.(i) <- (Utop_zed_char.unsafe_of_char '*', style)
    done;
    (text, pos)
...
class ['a] read_keyword ?history () = object(self)
  ...
  method! completion =
    let word = Utop_zed_rope.to_string self#input_prev in
    let keywords = List.filter (fun (keyword, _value) -> Utop_zed_string.starts_with ~prefix:word keyword) self#keywords in
    self#set_completion 0 (List.map (fun (keyword, _value) -> (keyword, Utop_zed_string.empty ())) keywords)
end
...
let newline = Utop_zed_char.unsafe_of_uChar @@ newline_uChar
...
let rec compute_position cols pos text start stop =
  if start = stop then
    ...
    if ch = newline then
      ...
      let width= Utop_zed_char.width ch in
...
let rec get_index_of_last_displayed_word column columns index words =
  match words with
    | [] ->
        index - 1
    | (word, _suffix) :: words ->
        let column = column + Utop_zed_string.length word in
...
(*let rec get_index_of_last_displayed_word_by_width column columns index words =
  match words with
    | [] ->
        index - 1
    | (word, _suffix) :: words ->
        let column = column + Utop_zed_string.(aval_width (width word)) in
        if column <= columns - 1 then
  ...
  let rec loop row col idx =
    if idx < Array.length str then begin
      ...
      if ch = newline then
        ...
        let width= max 1 (Utop_zed_char.width ch) in
...
object(self)
            ...
            | None ->
                ...
                let rec loop idx col = function
                  | [] ->
                      ()
                  | (word, _suffix) :: words ->
                      let len = Utop_zed_string.length word in
            ...
            if resolver = None then
              match key with
                | { control = false; meta = false; shift = false; code = Char ch } ->
                    Utop_zed_macro.add self#macro (Edit (LTerm_edit.Utop_zed (Utop_zed_edit.Insert (Utop_zed_char.unsafe_of_uChar ch))));
                    self#insert ch
  ...
  method private exec ?(keys= []) actions=
    ...
    | Accept :: _ when S.value self#mode = Edition ->
        Utop_zed_macro.add self#macro Accept;
        return (Result self#eval)
    | Clear_screen :: actions ->
        Utop_zed_macro.add self#macro Clear_screen;
    ...
    | Edit LTerm_edit.Play_macro :: actions ->
        Utop_zed_macro.cancel self#macro;
        self#exec ~keys (Utop_zed_macro.contents macro @ actions)
    | Suspend :: actions ->
        if Sys.win32 then
          self#exec ~keys actions
        else begin
    ...
    | Edit_with_external_editor :: actions -> begin
        ...
        let input = Utop_zed_rope.to_string (Utop_zed_edit.text self#edit) in
        Lwt_io.with_file ~mode:Output temp_fn (fun oc -> Lwt_io.write_line oc (Utop_zed_string.to_utf8 input))
        >>= fun () ->
        ...
        (if status <> WEXITED 0 then
           ...
           Lwt.try_bind
             ...
             (fun s  ->
                let s = Utop_zed_utf8.rstrip s in
                Utop_zed_edit.goto_bot self#context;
                Utop_zed_edit.replace self#context (Utop_zed_rope.length (Utop_zed_edit.text self#edit))
                  (Utop_zed_rope.of_string (Utop_zed_string.unsafe_of_utf8 s));
                Lwt.return ())
             (function
    ...
    let event =
      E.map_p
        ...
        (E.select [
           ...
           Utop_zed_edit.update self#edit [Utop_zed_edit.cursor self#context];
