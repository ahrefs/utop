...
(**
  ...
  Therefore the logical type definition of [Utop_zed_char.t] can be seen as
  {[
    type Utop_zed_char.t= {
      core: UChar.t;
...
val of_utf8 : ?indv_combining:bool -> string -> t
  (** [of_utf8 str] returns a [zed_char] from utf8 encoded [str].
    ...
    It will raise [Failure "malformed Utop_zed_char sequence"] If the validation
    is not passed.
    @param indv_combining allow to create a [Utop_zed_char.t] from a single
...
val mix_uChar : t -> Uchar.t -> (t, t) result
  (** [mix_uChar chr uChar] tries to append [uChar] to [chr] and returns
    ...
    [Error (Utop_zed_char.t consists of uChar)] is returned. *)
...
val of_uChars : ?trim:bool -> ?indv_combining:bool -> Uchar.t list -> t option * Uchar.t list
  (** [of_uChars uChars] transforms [uChars] to a tuple. The first value
    is an optional [Utop_zed_char.t] and the second is a list of remaining
    uChars. The optional [Utop_zed_char.t] is either a legal grapheme(a core
    ...
    @param indv_combining create a [Utop_zed_char] from an individual dissociated combining mark, default to [true]
  *)
...
val zChars_of_uChars : ?trim:bool -> ?indv_combining:bool -> Uchar.t list -> t list * Uchar.t list
  (** [zChars of_uChars uChars] transforms [uChars] to a tuple. The first
    value is a list of [Utop_zed_char.t] and the second is a list of remaining uChars.
    @param trim trim leading combining marks before transforming, default to [false]
    @param indv_combining create a [Utop_zed_char] from an individual dissociated combining mark, default to [true]
  *)
...
(** The prefix 'unsafe_' of [unsafe_of_char] and [unsafe_of_uChar] means
  ...
  the result to the edit engine. Other modules in zed, like Utop_zed_string,
  Utop_zed_lines, Utop_zed_edit ... are already well designed to deal with such a
...
val unsafe_of_char : char -> t
  (** [unsafe_of_char ch] returns a [Utop_zed_char] whose core is [ch]. *)

val unsafe_of_uChar : Uchar.t -> t
  (** [unsafe_of_uChar ch] returns a [Utop_zed_char] whose core is [ch]. *)
