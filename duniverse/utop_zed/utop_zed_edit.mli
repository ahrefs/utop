...
val update : 'a t -> Utop_zed_cursor.t list -> unit event
...
val mark : 'a t -> Utop_zed_cursor.t
...
val new_cursor : 'a t -> Utop_zed_cursor.t
...
val context : ?check : bool -> 'a t -> Utop_zed_cursor.t -> 'a context
...
val cursor : 'a context -> Utop_zed_cursor.t
...
val goto : 'a context -> ?set_wanted_column : bool -> int -> unit
  (** [goto ctx ?set_column position] moves the cursor to the given
      position. It raises {!Utop_zed_cursor.Out_of_bounds} if the position
...
val move : 'a context -> ?set_wanted_column : bool -> int -> unit
  (** [move ctx ?set_wanted_column delta] moves the cursor by the
      ...
      {!Utop_zed_cursor.Out_of_bounds} if the current plus [delta] is
...
val insert : 'a context -> Utop_zed_rope.t -> unit
...
val insert_no_erase : 'a context -> Utop_zed_rope.t -> unit
...
val replace : 'a context -> int -> Utop_zed_rope.t -> unit
