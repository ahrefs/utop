...
let of_rope rope =
  ...
  let rec loop zip (length, widths) acc =
    if Utop_zed_rope.Zip.at_eos zip then
      ...
      let ch, zip = Utop_zed_rope.Zip.next zip in
      if Uchar.to_int (Utop_zed_char.core ch) = 10 then
        let width, width_info= calc_widths widths in
        loop0 zip (append (append acc (String { length; width; width_info })) Return)
      else
        loop zip (length + 1, Utop_zed_char.width ch::widths) acc
  and loop0 zip acc =
    if Utop_zed_rope.Zip.at_eos zip then
      acc
    else
      ...
      if Uchar.to_int (Utop_zed_char.core ch) = 10 then
        ...
        loop zip (1, [Utop_zed_char.width ch]) acc
  in
  loop0 (Utop_zed_rope.Zip.make_f rope 0) empty
