...
let to_raw= Utop_zed_utf8.explode
let to_array t= Array.of_list (Utop_zed_utf8.explode t)

let zero= String.make 1 (Char.chr 0)

let core t= Utop_zed_utf8.unsafe_extract t 0
let combined t= List.tl (Utop_zed_utf8.explode t)
...
let prop t= prop_uChar (Utop_zed_utf8.unsafe_extract t 0)
...
let length= Utop_zed_utf8.length
let size= length

let width t= Uucp.Break.tty_width_hint (Utop_zed_utf8.unsafe_extract t 0)

let out_of_range t i= i < 0 || i >= size t
let get= Utop_zed_utf8.get
...
let append ch mark=
  ...
  | Printable 0-> ch ^ (Utop_zed_utf8.singleton mark)
  | _-> failwith "combining mark expected"

let compare_core t1 t2=
  let core1= Utop_zed_utf8.unsafe_extract t1 0
  and core2= Utop_zed_utf8.unsafe_extract t2 0 in
  Uchar.compare core1 core2

let compare_raw= Utop_zed_utf8.compare
...
let mix_uChar zChar uChar=
  match prop_uChar uChar with
  | Printable 0->
    Ok (zChar ^ (Utop_zed_utf8.singleton uChar))
  | _->
    Error (Utop_zed_utf8.singleton uChar)
  ...
  | uChar::tl->
    ...
    | None, _->
      if indv_combining then
        Some (Utop_zed_utf8.singleton uChar), tl
      else
        None, uChars
    | Some (Printable _w, uChar), tl->
      let combined, tl= subsequent tl in
      Some (Utop_zed_utf8.implode (uChar::combined)), tl
    | Some (Null, uChar), tl->
      Some (Utop_zed_utf8.singleton uChar) ,tl
    | Some (Other, uChar), tl->
      Some (Utop_zed_utf8.singleton uChar) ,tl
...
let unsafe_of_utf8 : string -> t=
  fun str-> if String.length str > 0
    then str
    else failwith "malformed Utop_zed_char sequence"
let of_utf8 ?(indv_combining=true) str=
  match of_uChars ~indv_combining (Utop_zed_utf8.explode str) with
  | Some zChar, []-> zChar
  | _-> failwith "malformed Utop_zed_char sequence"

let to_utf8 : t -> string= id

let unsafe_of_char c=
  Utop_zed_utf8.singleton (Uchar.of_char c)

let unsafe_of_uChar uChar= Utop_zed_utf8.singleton uChar

let for_all= Utop_zed_utf8.for_all
let iter= Utop_zed_utf8.iter
