...
type t = {
  ...
  get_lines : unit -> Utop_zed_lines.t;
...
let create length changes get_lines position wanted_column =
  ...
  let compute_coordinates_and_display position =
    ...
    let index = Utop_zed_lines.line_index lines position in
    let bol= Utop_zed_lines.line_start lines index in
    let column= position - bol in
    let width= Utop_zed_lines.force_width lines bol column in
