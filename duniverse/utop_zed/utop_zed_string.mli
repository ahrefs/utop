...
exception Invalid of string * string
  (** [Invalid (error, text)] Exception raised when an invalid Utop_zed_char
...
type seg_width = { start : int; len : int; width : int; }
  (** Type of the width of a segment of a Utop_zed_string.t *)

type all_width = { len : int; width : int; }
  (** Type of the width of a whole Utop_zed_string.t *)

type width = (all_width, seg_width) result
  (** Type of the width of a Utop_zed_string.t *)

type t
  (** Type of Utop_zed_string.t *)

val unsafe_of_utf8 : string -> t
  (** Create a Utop_zed_string.t from a utf8 encoded string. *)

val of_utf8 : string -> t
  (** Create a Utop_zed_string.t from a utf8 encoded string and check whether it's well formed.
    @raise Invalid
    @raise Utop_zed_utf8.Invalid
   *)

val to_utf8 : t -> string
  (** Create a utf8 encoded string from a Utop_zed_string.t. *)

val explode : t -> Utop_zed_char.t list
  (** [explode str] returns the list of all Utop_zed_char.t of [str]. *)

val rev_explode : t -> Utop_zed_char.t list
  (** [explode str] returns the list of all Utop_zed_char.t of [str] in reverse order. *)

val unsafe_explode : t -> Utop_zed_char.t list
  (** [explode str] returns the list of all Utop_zed_char.t of [str] even if [str] is malformed. *)

val unsafe_rev_explode : t -> Utop_zed_char.t list
  (** [explode str] returns the list of all Utop_zed_char.t of [str] in reverse order even if [str] is malformed. *)

val implode : Utop_zed_char.t list -> t
  (** [implode l] returns the concatenation of all Utop_zed_char.t of [l]. *)

val aval_width : width -> int
  (** Returns the widest available width *)

val init : int -> (int -> Utop_zed_char.t) -> t
...
val make : int -> Utop_zed_char.t -> t
  (** [make n ch] creates a Utop_zed_string.t of length [n] filled with [ch]. *)

val copy : t -> t
  (** [copy s] returns a copy of [s], that is, a fresh Utop_zed_string.t containing the same elements as [s]. *)
...
type index = int
val get : t -> int -> Utop_zed_char.t
  (** [get str idx] returns the Utop_zed_char.t at index [idx] in [str]. *)

val get_raw : t -> int -> Uchar.t
  (** [get_raw str idx] returns the Uchar.t at Uchar.t based index [idx] in [str]. *)

val empty : unit -> t
  (** [empty ()] creates an empty Utop_zed_string.t. *)

val width_ofs : ?start:index -> ?num:int -> t -> width
(** [width_ofs ?start ?num str] returns the [width] of a Utop_zed_string.t that starts at offset [start] and has length less than [num]. *)

val width : ?start:int -> ?num:int -> t -> width
(** [width ?start ?num str] returns the [width] of a Utop_zed_string.t that starts at positon [start] and has length less than [num]. *)
...
val length : t -> int
  (** [length str] returns the number of Utop_zed_char.t in [str] *)
...
val extract : t -> index -> Utop_zed_char.t
  (** [extract str ofs] returns the Utop_zed_char.t at offset [ofs] in [str]. *)

val extract_next : t -> index -> (Utop_zed_char.t * index)
  (** [extract_next str ofs] returns the Utop_zed_char.t at offset [ofs] in [str] and the offset of the next Utop_zed_char.t *)

val extract_prev : t -> index -> (Utop_zed_char.t * index)
  (** [extract_prev str ofs] returns the Utop_zed_char.t at the previous offset [ofs] in [str] and this offset. *)
...
val of_uChars : Uchar.t list -> t * Uchar.t list
  (** [of_uChars l] returns a tuple of which the first element is a well formed Utop_zed_string.t concatenating of all Uchar.t of [l] and the second element is a list of the remaining Uchar.t. *)

val for_all : (Utop_zed_char.t -> bool) -> t -> bool
  (** [for_all p zStr] checks if all Utop_zed_char.t in [zStr]
   satisfy the predicate [p]. *)


val iter : (Utop_zed_char.t -> unit) -> t -> unit
  (** [iter f str] applies [f] an all characters of [str] starting from the left. *)

val rev_iter : (Utop_zed_char.t -> unit) -> t -> unit
  (** [iter f str] applies [f] an all characters of [str] starting from the right. *)

val fold : (Utop_zed_char.t -> 'a -> 'a) -> t -> 'a -> 'a
  (** [fold f str acc] applies [f] on all characters of [str] starting from the left, accumulating a value. *)

val rev_fold : (Utop_zed_char.t -> 'a -> 'a) -> t -> 'a -> 'a
  (** [fold f str acc] applies [f] on all characters of [str] starting from the right, accumulating a value. *)

val map : (Utop_zed_char.t -> Utop_zed_char.t) -> t -> t
  (** [map f str] maps all characters of [str] with [f]. *)

val rev_map : (Utop_zed_char.t -> Utop_zed_char.t) -> t -> t
...
val compare : t -> t -> int
  (** Compares two strings by [Utop_zed_char.compare]. *)
...
module Buf :
  sig
    type buf
      (** Type of Utop_zed_string buffers. *)

    val create : int -> buf
      (** Create a new empty buffer. *)

    val contents : buf -> t
      (** [contents buffer] returns the contents of [buffer] as a Utop_zed_string.t. *)
    ...
    val add_zChar : buf -> Utop_zed_char.t -> unit
