...
val get : rope -> int -> Utop_zed_char.t
...
val iter_leaf : (Utop_zed_string.t -> unit) -> rope -> unit
  (** [iter_leaf f rope] applies [f] on all leaves of [rope] starting
      from the left. *)

val rev_iter_leaf : (Utop_zed_string.t -> unit) -> rope -> unit
  (** [iter_leaf f rope] applies [f] on all leaves of [rope] starting
      from the right. *)

val fold_leaf : (Utop_zed_string.t -> 'a -> 'a) -> rope -> 'a -> 'a
  (** [fold f rope acc] applies [f] on all leaves of [rope] starting
      from the left, accumulating a value. *)

val rev_fold_leaf : (Utop_zed_string.t -> 'a -> 'a) -> rope -> 'a -> 'a
...
module Zip : sig
  ...
  val find_f : (Utop_zed_char.t -> bool) -> t -> t
  ...
  val find_b : (Utop_zed_char.t -> bool) -> t -> t
...
module Buffer :
  sig
    ...
    val add : t -> Utop_zed_char.t -> unit
    ...
    val add_string : t -> Utop_zed_string.t -> unit
...
val init : int -> (int -> Utop_zed_char.t) -> rope
val init_from_uChars : int -> (int -> Uchar.t) -> rope
val of_string : Utop_zed_string.t -> rope
val to_string : rope -> Utop_zed_string.t
