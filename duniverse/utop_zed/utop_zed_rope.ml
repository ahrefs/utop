...
type t=
  ...
  | Leaf of Utop_zed_string.t * (int * int)
    (* [Leaf(str, (len, size))] *)
...
let empty ()= Leaf (Utop_zed_string.empty (), (0, 0))
...
let rec trim_hd t=
  match t with
  | Leaf (str, (l, _))->
    let hd, _= Utop_zed_string.extract_next str 0 in
    let hd= hd
      |> Utop_zed_char.to_utf8
      |> Utop_zed_string.unsafe_of_utf8
    in
    let after= Utop_zed_string.after str 1 in
    let size= Utop_zed_string.size after in
...
let append_cm t cm=
  let size= Utop_zed_string.size cm in
  let rec append_cm t=
    match t with
    | Leaf (str, (l, s))-> Leaf (Utop_zed_string.append str cm, (l, s + size))
...
let rec unsafe_get_raw idx rope =
  match rope with
    | Leaf(text, _) ->
        Utop_zed_string.get_raw text idx
...
let append rope1 rope2 =
  let len_12_comb=
    if length rope1 > 0 && length rope2 > 0 then
      Utop_zed_char.is_combining_mark (Utop_zed_char.core (get rope2 0))
    else
      false
  in
  ...
  match rope1, rope2 with
    ...
    | Leaf(text1, (len1, size1)), Leaf(text2, (len2, size2))
      when len12 len1 len2 <= max_leaf_size ->
        Leaf(Utop_zed_string.append text1 text2, (len12 len1 len2, size1+size2))
    | Node(d, len_l, rope_l, _, Leaf(text1, (len1,size1))), Leaf(text2, (len2,size2))
      ...
      let ls= len12 len1 len2, size1+size2 in
        Node(
          ...
          rope_l,
          ls,
          Leaf(Utop_zed_string.append text1 text2, ls))
    ...
    | _ ->
      let rope1, rope2=
        if length rope1 > 0 && length rope2 > 0 then
          if Utop_zed_char.is_combining_mark (Utop_zed_char.core (get rope2 0)) then
...
let rec unsafe_sub rope idx len =
  match rope with
    | Leaf(text, _) ->
      let str= Utop_zed_string.sub ~pos:idx ~len text in
      let size= Utop_zed_string.size str in
        Leaf(str, (len,size))
...
let make length char =
  if length < max_leaf_size then
    Leaf(Utop_zed_string.make length char, (length, length))
  else begin
    let text = Utop_zed_string.make max_leaf_size char in
    let chunk = Leaf(text, (max_leaf_size, max_leaf_size)) in
    let rec loop acc n =
      if n = 0 then
        acc
      else if n < max_leaf_size then
        let str= Utop_zed_string.sub ~pos:0 ~len:n text in
        let size= Utop_zed_string.size str in
...
let singleton ch =
  Leaf(Utop_zed_string.make 1 ch, (1, 1))
    ...
    if Utop_zed_char.is_combining_mark ch then
      if length rope = 0 then
        ...
        if pos = 0 then
          ...
          if Utop_zed_char.is_printable_core (Utop_zed_char.core glyph) then
            let glyph= Utop_zed_char.append glyph ch in
            replace rope pos 1 (Leaf (Utop_zed_string.implode [glyph], (1, 1)))
          else
            failwith "inserting an individual combining mark"
    else
      let sub= Leaf (Utop_zed_string.implode [Utop_zed_char.unsafe_of_uChar ch], (1, 1)) in
      insert rope pos sub
...
let rec rev_iter f = function
  | Leaf(text, _) ->
      Utop_zed_string.rev_iter f text
  | Node(_, _, rope_l, _, rope_r) ->
      rev_iter f rope_r;
      rev_iter f rope_l
...
let rec fold f rope acc =
  match rope with
    | Leaf(text, _) ->
        Utop_zed_string.fold f text acc
    | Node(_, _, rope_l, _, rope_r) ->
...
let rec rev_fold f rope acc =
  match rope with
    | Leaf(text, _) ->
        Utop_zed_string.rev_fold f text acc
    | Node(_, _, rope_l, _, rope_r) ->
        rev_fold f rope_l (rev_fold f rope_r acc)

let rec map f = function
  | Leaf(txt, len) ->
      Leaf(Utop_zed_string.map f txt, len)
...
let rec rev_map f = function
  | Leaf(txt, len) ->
      Leaf(Utop_zed_string.rev_map f txt, len)
  | Node(depth, length_l, rope_l, length_r, rope_r) ->
...
let rec cmp_loop str1 ofs1 str2 ofs2 rest1 rest2 =
  if ofs1 = Utop_zed_string.bytes str1 then
    match rest1 with
      | [] ->
          if ofs2 = Utop_zed_string.length str2 && rest2 = [] then
            0
          else
            -1
      | rope1 :: rest1 ->
          cmp_search1 rope1 str2 ofs2 rest1 rest2
  else if ofs2 = Utop_zed_string.bytes str2 then
    match rest2 with
      | [] ->
          1
      | rope2 :: rest2 ->
    ...
    let chr1, ofs1 = Utop_zed_string.extract_next str1 ofs1
    and chr2, ofs2 = Utop_zed_string.extract_next str2 ofs2 in
    let d = Utop_zed_char.compare_raw chr1 chr2 in
    if d = 0 then
...
module Zip = struct
  ...
  let rec make_rec ofs rope pos rest_b rest_f =
    match rope with
    | Leaf(str, _) ->
      { idx= Utop_zed_string.move str 0 pos;
        pos = pos;
  ...
  let rec next_leaf ofs rope rest_b rest_f =
    match rope with
    | Leaf(str, _) ->
      let chr, idx= Utop_zed_string.extract_next str 0 in
      (chr,
       { idx;
         pos = 1;
  ...
  let next zip =
    if zip.idx = Utop_zed_string.bytes zip.zip.str then
      ...
      let chr, idx= Utop_zed_string.extract_next zip.zip.str zip.idx in
  ...
  let rec prev_leaf ofs rope rest_b rest_f =
    ...
    | Leaf(str, (len,_size)) ->
      let chr, idx=
        Utop_zed_string.extract_prev str (Utop_zed_string.bytes str)
      in
      (chr,
       { idx;
         pos = len - 1;
  ...
  let prev zip =
    if zip.pos = 0 then
      ...
      let chr, idx= Utop_zed_string.extract_prev zip.zip.str zip.idx in
      (chr, { zip with idx; pos = zip.pos - 1 })
  ...
  let rec move_f n ofs rope rest_b rest_f =
    ...
    | Leaf(str, (len,_size)) ->
      if n <= len then
        { idx= Utop_zed_string.move str 0 n;
          pos = n;
          zip = { str; ofs; leaf = rope; rest_b; rest_f } }
      else begin
        match rest_f with
  ...
  let rec move_b n ofs rope rest_b rest_f =
    ...
    | Leaf(str, (len,_size)) ->
      if n <= len then
        { idx= Utop_zed_string.move str (Utop_zed_string.bytes str) (-n);
          pos = len - n;
          zip = { str; ofs; leaf = rope; rest_b; rest_f } }
      else begin
        match rest_b with
  ...
  let move n zip =
    if n > 0 then
      ...
      if zip.pos + n <= len then
        { zip with
          idx= Utop_zed_string.move zip.zip.str zip.idx n;
          pos = zip.pos + n }
      else
        match zip.zip.rest_f with
        | [] ->
  ...
  let at_eos zip= zip.zip.rest_f = [] && zip.idx = Utop_zed_string.bytes zip.zip.str
  ...
  let sub zip len =
    if len < 0 then
      ...
      if len <= len' then
        let str= Utop_zed_string.sub ~pos:zip.pos ~len zip.zip.str in
        let size= Utop_zed_string.size str in
        Leaf(str, (len,size))
      else
        let str= Utop_zed_string.sub ~pos:zip.pos ~len:(Utop_zed_string.length zip.zip.str - zip.pos) zip.zip.str in
        let size= Utop_zed_string.size str in
...
module Zip_raw = struct
  ...
  let rec make_f_rec ofs rope pos rest_b rest_f =
    match rope with
    | Leaf(str, _) ->
      { idx= Utop_zed_string.move_raw str 0 pos;
        pos = pos;
        zip = { str; ofs = ofs - pos; leaf = rope; rest_b; rest_f } }
    | Node(_, _, r1, _, r2) ->
  ...
  let rec make_b_rec ofs rope pos rest_b rest_f =
    match rope with
      | Leaf(str, (len,_)) ->
          { idx= Utop_zed_string.move_raw str (Utop_zed_string.bytes str) (- (len - pos));
            pos = pos;
            zip = { str; ofs = ofs - pos; leaf = rope; rest_b; rest_f } }
      | Node(_, _, r1, _, r2) ->
          let len1 = length r1 in
  ...
  let rec next_leaf ofs rope rest_b rest_f =
    match rope with
    | Leaf(str, _) ->
      let chr, idx= Utop_zed_utf8.unsafe_extract_next (Utop_zed_string.to_utf8 str) 0 in
      (chr,
       { idx;
         pos = 1;
         zip = { str; ofs; leaf = rope; rest_b; rest_f } })
  ...
  let next zip =
    if zip.pos = Utop_zed_string.size zip.zip.str then
      match zip.zip.rest_f with
      | [] ->
        raise Out_of_bounds
      | rope :: rest ->
      ...
      let chr, idx= Utop_zed_utf8.unsafe_extract_next (Utop_zed_string.to_utf8 zip.zip.str) zip.idx in
      (chr, { zip with idx; pos = zip.pos + 1 })
  ...
  let rec prev_leaf ofs rope rest_b rest_f =
    ...
    | Leaf(str, (_len, size)) ->
      let chr, idx =
        let str= Utop_zed_string.to_utf8 str in
        Utop_zed_utf8.unsafe_extract_prev str (String.length str)
      in
      (chr,
       { idx;
         pos = size - 1;
  ...
  let prev zip =
    if zip.pos = 0 then
      ...
      let chr, idx= Utop_zed_utf8.unsafe_extract_prev (Utop_zed_string.to_utf8 zip.zip.str) zip.idx in
      (chr, { zip with idx; pos = zip.pos - 1 })
  ...
  let rec move_f n ofs rope rest_b rest_f =
    ...
    | Leaf(str, (_,size)) ->
      if n <= size then
        { idx= Utop_zed_string.move_raw str 0 n;
          pos = n;
          zip = { str; ofs; leaf = rope; rest_b; rest_f } }
      else begin
        match rest_f with
  ...
  let rec move_b n ofs rope rest_b rest_f =
    ...
    | Leaf(str, (_,size)) ->
      if n <= size then
        { idx= Utop_zed_string.move_raw str (Utop_zed_string.bytes str) (-n);
          pos = size - n;
          zip = { str; ofs; leaf = rope; rest_b; rest_f } }
      else begin
        match rest_b with
  ...
  let move n zip =
    if n > 0 then
      ...
      if zip.pos + n <= size then
        { zip with
          idx= Utop_zed_string.move_raw zip.zip.str zip.idx n;
          pos = zip.pos + n }
      else
        match zip.zip.rest_f with
        | [] ->
...
module Buffer = struct
  type t = {
    mutable acc : rope;
    mutable buf : Utop_zed_string.Buf.buf;
    mutable idx : int;
  }

  let create () = {
    acc = empty ();
    buf = Utop_zed_string.Buf.create 1024;
    idx = 0;
  }

  let add buffer x =
    if buffer.idx = max_leaf_size then begin
      let str= Utop_zed_string.Buf.contents buffer.buf in
      let size= Utop_zed_string.size str in
      buffer.acc <- append buffer.acc (Leaf(str, (max_leaf_size,size)));
      Utop_zed_string.Buf.reset buffer.buf;
      Utop_zed_string.Buf.add_zChar buffer.buf x;
      buffer.idx <- Utop_zed_string.Buf.length buffer.buf
    end else begin
      Utop_zed_string.Buf.add_zChar buffer.buf x;
      buffer.idx <- Utop_zed_string.Buf.length buffer.buf
    end
  ...
  let add_uChar buffer x =
    if buffer.idx = max_leaf_size then begin
      ...
      buffer.acc <- append buffer.acc (Leaf(str, (max_leaf_size,size)));
      Utop_zed_string.Buf.reset buffer.buf;
      Utop_zed_string.Buf.add_uChar buffer.buf x;
      buffer.idx <- Utop_zed_string.Buf.length buffer.buf
    end else begin
      Utop_zed_string.Buf.add_uChar buffer.buf x;
      buffer.idx <- Utop_zed_string.Buf.length buffer.buf
  ...
  let add_string buf str= Utop_zed_string.iter (add buf) str
...
let init_from_uChars len f=
  ...
  | len when len > 0 ->
    ...
    let zChars, _= Utop_zed_char.zChars_of_uChars uChars in
    let buf = Buffer.create () in
    List.iter (Buffer.add buf) zChars;
    Buffer.contents buf
  | _-> raise (Invalid_argument "Utop_zed_rope.init_from_uChars")
...
let rec to_string t=
  match t with
  | Leaf (s,_)-> s
  | Node (_,_,l,_,r)-> Utop_zed_string.append (to_string l) (to_string r)
