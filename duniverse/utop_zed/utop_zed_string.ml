...
module Utop_zed_string0 = struct
  ...
  let size str= Utop_zed_utf8.length str
  ...
  let unsafe_next str ofs=
    ...
    let rec skip str ofs=
      if ofs >= str_len then
        ...
        let chr, next= Utop_zed_utf8.unsafe_extract_next str ofs in
        if Utop_zed_char.is_combining_mark chr then
          skip str next
        else
          ofs
    in
    if ofs < 0 || ofs >= String.length str then
      ...
      let chr, next= Utop_zed_utf8.unsafe_extract_next str ofs in
      if Utop_zed_char.is_printable chr then
        skip str next
      else
        next
  ...
  let next_ofs str ofs=
    ...
    if ofs < 0 || ofs >= String.length str then
      ...
      let chr, next= Utop_zed_utf8.unsafe_extract_next str ofs in
      if Utop_zed_char.is_printable_core chr then
        skip str next
      else if Utop_zed_char.is_combining_mark chr then
  ...
  let unsafe_prev str ofs=
    let rec skip str ofs=
      if ofs = 0 then
        ...
        let chr, prev= Utop_zed_utf8.unsafe_extract_prev str ofs in
        if Utop_zed_char.is_combining_mark chr then
          skip str prev
        else
          prev
    ...
    if ofs <= 0 || ofs > String.length str then
      ...
      let chr, prev= Utop_zed_utf8.extract_prev str ofs in
      if Utop_zed_char.is_combining_mark chr then
        skip str prev
      else
        prev
  ...
  let prev_ofs str ofs=
    ...
    if ofs <= 0 || ofs > String.length str then
      ...
      if Utop_zed_char.is_combining_mark chr then
        ...
        if prev = 0 then
          if Utop_zed_char.is_printable_core (Utop_zed_utf8.unsafe_extract str 0) then
            prev
          else
            fail str 0 "individual combining marks encountered"
        else
          let chr, next= Utop_zed_utf8.unsafe_extract_next str prev in
          match Utop_zed_char.prop_uChar chr with
          | Printable 0
          | Other
  ...
  let rec move_l_raw str ofs len=
    if len = 0 then
      ...
      move_l_raw str (Utop_zed_utf8.unsafe_next str ofs) (len - 1)
  ...
  let move_b_raw str ofs len=
    let rec move str ofs len=
      if len = 0 then
        ...
        move str (Utop_zed_utf8.unsafe_prev str ofs) (len - 1)
    in
    if ofs < 0 || ofs > String.length str then
      raise Out_of_bounds
    else
  ...
  let extract str ofs=
    ...
    Utop_zed_char.unsafe_of_utf8 (String.sub str ofs (next - ofs))

  let extract_next str ofs=
    let next= next_ofs str ofs in
    (Utop_zed_char.unsafe_of_utf8 (String.sub str ofs (next - ofs)), next)

  let extract_prev str ofs=
    let prev= prev_ofs str ofs in
    (Utop_zed_char.unsafe_of_utf8 (String.sub str prev (ofs - prev)), prev)

  let to_raw_list str= Utop_zed_utf8.explode str
  ...
  let width_ofs ?(start=0) ?num str=
    ...
    let rec calc w idx ofs=
      if ofs < str_len then
        ...
        let chr_width= Utop_zed_char.width chr in
        if chr_width > 0 then
  ...
  let implode chars=
    String.concat "" (List.map Utop_zed_char.to_utf8 chars)

  let init len (f: int -> Utop_zed_char.t)=
  ...
  let init_from_uChars len f=
    ...
    | len when len > 0 ->
      ...
      implode zChars
    | _-> raise (Invalid_argument "Utop_zed_string0.init_from_uChars")


  let unsafe_of_uChars uChars=
    match uChars with
    | []-> ""
    | _-> String.concat "" (List.map Utop_zed_utf8.singleton uChars)

  let of_uChars uChars=
    match uChars with
    | []-> "", []
    | fst::_->
      if Utop_zed_char.is_combining_mark fst then
        ("", uChars)
      else
        (uChars |> List.map Utop_zed_utf8.singleton |> String.concat "", [])
  ...
  let append s1 s2=
    let validate_s2 ()=
      let s2_first= Utop_zed_utf8.unsafe_extract s2 0 in
      if Utop_zed_char.is_combining_mark s2_first then
    ...
    if s1 = "" then
      ...
      if Utop_zed_char.(is_printable_core (core s1_last)) then
        unsafe_append s1 s2
      else
  ...
  let of_utf8 : string -> t= fun str->
    ...
    else if Utop_zed_char.is_combining_mark (Utop_zed_utf8.extract str 0) then
      fail str 0 "individual combining marks encountered"
    else
      unsafe_of_utf8 str
  ...
  let look str ofs= Utop_zed_utf8.extract str ofs

  let nth t n= if check_range t n
    then n
    else raise (Invalid_argument "Utop_zed_string.nth")

  let next t n=
    let n= n + 1 in
    if check_range t n
    then n
    else raise (Invalid_argument "Utop_zed_string.next")

  let prev t n=
    let n= n - 1 in
    if check_range t n
    then n
    else raise (Invalid_argument "Utop_zed_string.prev")
  ...
  let compare str1 str2= Utop_zed_utils.list_compare
    ~compare:Utop_zed_char.compare_raw
  ...
  let sub_ofs ~ofs ~len s=
    if ofs < 0 || len < 0 || ofs > bytes s - len then
      invalid_arg "Utop_zed_string.sub"
    else
  ...
  let ends_with ~suffix str=
    Utop_zed_utf8.ends_with str suffix
  ...
  module Buf0 = struct
    ...
    let add_zChar b zChar=
      Buffer.add_string b (Utop_zed_char.to_utf8 zChar)

    let add_uChar b uChar=
      Buffer.add_string b (Utop_zed_utf8.singleton uChar)
